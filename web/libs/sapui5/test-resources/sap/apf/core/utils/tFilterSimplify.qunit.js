...
(function () {
    ...
    var CollectPropertiesAndValuesVisitor = sap.apf.core.utils.CollectPropertiesAndValuesVisitor;
    ...
    function createEmptyFilter(context) {
        return new sap.apf.core.utils.Filter(context.messageHandler);
    }
    ...
    function createFilterPointingToEmptyFilter(context) {
        ...
        return new sap.apf.core.utils.Filter(context.messageHandler, child);
    }
    ...
    function eqA(context, value) {
        return new sap.apf.core.utils.Filter(context.messageHandler, "A", context.EQ, value);
    }

    function eq(name, value, mh) {
        return new sap.apf.core.utils.Filter(mh, name, sap.apf.core.constants.FilterOperators.EQ, value);
    }
    ...
    function ToStringVisitor() {
        ...
        this.processTerm = function(term) {
            switch (term.getOp()) { // all:
                ...
                case sap.apf.core.constants.FilterOperators.BT:
    ...
    function createDisjunctionAEq(context, valueList) {
        var filter0 = new sap.apf.core.utils.Filter(context.messageHandler, "A", context.EQ,
            valueList[0]);
        var rest = valueList.slice(1);
        var filter;
        rest.forEach(function (value) {
            filter = new sap.apf.core.utils.Filter(context.messageHandler, "A", context.EQ, value);
    ...
    QUnit.test("CollectPropertiesAndValuesVisitor WHEN AND equation and empty filter", function (assert) {
        ...
        var equ2 = eqA(this, 2);
        var left = new sap.apf.core.utils.Filter(this.messageHandler, equ1);
        left.addAnd(equ2);
        var top = new sap.apf.core.utils.Filter(this.messageHandler, left);
    ...
    function createAndOrOrOverEqA(context) {
        ...
        var filter = new sap.apf.core.utils.Filter(context.messageHandler, filter1);
    ...
    QUnit.test("CollectChildrenOfAndNodeVisitor WHEN input empty filter", function (assert) {
        var filter = createEmptyFilter(this);
        var collectFilterOfAnd = filter.traverse(new sap.apf.core.utils.CollectChildrenOfAndNodeVisitor());
    ...
    QUnit.test("CollectChildrenOfAndNodeVisitor of a filter which has 1 left expression (AND-filter) and 0 rest expression", function (assert) {
        ...
        var filter = new sap.apf.core.utils.Filter(this.messageHandler, leftFilter);
        var collectFilterOfAnd = filter.traverse(new sap.apf.core.utils.CollectChildrenOfAndNodeVisitor());
        assert.strictEqual(collectFilterOfAnd.length, 2, 'then return an array of 2 children filter');
    });
    ...
    QUnit.test("Basic POC of filter reduction by applying the context to a filter in normal form, using Filter.isConsistentWithFilter", function (assert) {
        var sbFilter = new sap.apf.core.utils.Filter(this.messageHandler, "A", this.BT, 10, 20); // assumed received from external
    ...
    QUnit.test("intersection of two transforms [] []", function (assert) {
        ...
        var filterReduction = new sap.apf.core.utils.FilterReduction();
    ...
    QUnit.test("CollectChildrenOfAndNodeVisitor.process() input FilterTerm THEN returns array)", function (assert) {
        var filter = new sap.apf.core.utils.Filter(this.messageHandler, "A", this.BT, 1, 5);
        ...
        assert.strictEqual(aFilters instanceof sap.apf.core.utils.Filter, false, 'THEN not a Filter or FilterTerm');
    });
    ...
    QUnit.test("CollectChildrenOfAndNodeVisitor.process() AND(A BT 1..5, deep emptyFilter THEN returns array)", function (assert) {
        ...
        assert.strictEqual(aFilters instanceof Array, true, 'THEN returns an array, not a Filter or FilterTerm');
        assert.strictEqual(aFilters instanceof sap.apf.core.utils.Filter, false, 'THEN not a Filter or FilterTerm');
        assert.strictEqual(aFilters instanceof sap.apf.core.utils.FilterTerm, false, 'THEN not a Filter or FilterTerm');
    });
    ...
    QUnit.test("filterSeparation a filter with a start filter, 1 well-formed disjunction, 1 filter term, 1 filter contains 1 term", function (assert) {
        ...
        var filterTerm = new sap.apf.core.utils.FilterTerm(this.messageHandler, "B", this.EQ, 1);
        var property = 'C';
        var obj = [];
        obj[property] = 1;

        var filterOneTerm = sap.apf.core.utils.Filter.createFromArray(this.messageHandler, [property], [obj], [0]);
    ...
    QUnit.test('filterSeparation AND(AND(eq,empty), empty) - AND in left branch', function (assert) {
        ...
        var separation = filterSimplify.filterSeparation(filter, this.messageHandler);
    ...
    QUnit.test('filterSeparation AND(OR(A=1,A=2,A=1,A=4), OR(A=1,A=2,C=42)) - right branch is external context', function (assert) {
        ...
        var filterSimplify = new sap.apf.core.utils.FilterReduction();
        var separated = filterSimplify.filterSeparation(filter, this.messageHandler);
    ...
    QUnit.test('FT(A=1) - FilterTerm directly intentionally is not processed', function (assert) {
        var mh = this.messageHandler;
        var filter = new sap.apf.core.utils.FilterTerm(mh, 'A', this.EQ, '1');
    ...
    QUnit.test('FT(A=1) - Filter(FilterTerm)', function (assert) {
        var mh = this.messageHandler;
        var term = new sap.apf.core.utils.FilterTerm(mh, 'A', this.EQ, '1');
        var filter = new sap.apf.core.utils.Filter(mh, term);
        ...
        assert.strictEqual(disjunctions.length, 1, '#disjunctions');
        assert.strictEqual(disjunctions[0] instanceof sap.apf.core.utils.Filter, true, 'disjunctions[0] instanceof Filter');
    ...
    QUnit.test("applyStartFilter with startFilter, transformed is an array of 2 transforms", function (assert) {
        var startFilter = new sap.apf.core.utils.Filter(this.messageHandler, "A", this.BT, 1, 5);
        var transform1 = {property: "A", values: [1, 2, 4, 5, 6, 7, 8]};
    ...
    QUnit.test("applyStartFilter with startFilter, transformed is an array of 1 transform, same property, no common value", function (assert) {
        var startFilter = new sap.apf.core.utils.Filter(this.messageHandler, "A", this.BT, 10, 15);
        var transform1 = {property: "A", values: [1, 2, 4, 5, 6, 7, 8]};
        var aTransformed = [transform1];
        var filterReduction = new sap.apf.core.utils.FilterReduction();
        var aSimplifiedTransformed = filterReduction.applyStartFilter(startFilter, aTransformed);
    ...
    QUnit.test("containsContradiction, contradiction found", function (assert) {
        ...
        var filterSimplify = new sap.apf.core.utils.FilterReduction();
        var containsAContradiction = filterSimplify.containsContradiction(this.messageHandler, aTransforms);
    ...
    QUnit.test("filterReduction with context A BT 1..5 AND AND(OR A=1,2,4,5,6,8): returns reduced OR and only 1 filter on 'A'", function (assert) {
        ...
        var between = new sap.apf.core.utils.Filter(this.messageHandler, "A", this.BT, 1, 5);
        var orNode = createDisjunctionAEq(this, [1, 4, 5, 6, 7, 8]);
        oFilter = oFilter.addAnd(between).addAnd(orNode);
        var filterSimplify = new sap.apf.core.utils.FilterReduction();
        var reducedFilter = filterSimplify.filterReduction(this.messageHandler, oFilter);
    ...
    QUnit.test("filterReduction without context, AND(OR,OR): returns only 1 OR over 'A' (AND eliminated) and OR is reduced", function (assert) {
        ...
        var reducedFilter = filterSimplify.filterReduction(this.messageHandler, pathFilter);

        assertFilterOverSingleProperty(reducedFilter, 'A', 1, assert);
        assert.notOk(filterSimplify.isContradicted(), "No contradiction found");
    });

    QUnit.test("filterReduction with context but no common property AND(OR,OR)", function (assert) {
        var startFilter = new sap.apf.core.utils.Filter(this.messageHandler, "B", this.BT, 1, 5);
        var aValues1 = [1, 2, 4, 5, 6, 7, 8];
        var aValues2 = [2, 3, 4, 7, 9];
        var filter1 = createDisjunctionAEq(this, aValues1);
        var filter2 = createDisjunctionAEq(this, aValues2);
    ...
    QUnit.test("filterReduction, AND(OR, OR) which returns a contradiction w/o context", function (assert) {
        ...
        var filter1 = createDisjunctionAEq(this, aValues1);
        var filter2 = createDisjunctionAEq(this, aValues2);
        var oFilter = new sap.apf.core.utils.Filter(this.messageHandler, filter1);
        ...
        assert.ok(filterSimplify.isContradicted(), "filterReduction found a contradiction");
    });
    ...
    QUnit.test('filterReduction EmptyFilter returns EmptyFilter', function (assert) {
        ...
        var filterSimplify = new sap.apf.core.utils.FilterReduction();
        var reducedFilter = filterSimplify.filterReduction(this.messageHandler, filter);
    ...
    QUnit.test('filterReduction OR(eq, eq) returns the same', function (assert) {
        var filter = new sap.apf.core.utils.Filter(this.messageHandler, 'A', this.EQ, 1);
        filter.addOr(new sap.apf.core.utils.Filter(this.messageHandler, 'B', this.EQ, 2));
        var filterSimplify = new sap.apf.core.utils.FilterReduction();
    ...
    QUnit.test('filterReduction AND(A<7), AND(A<5),AND(A=1, A=6)) returns A=1', function (assert) {
		...
		function lt(name, value){
			return new sap.apf.core.utils.Filter(mh, name, sap.apf.core.constants.FilterOperators.LT, value);
		}
    ...
    QUnit.test('filterReduction AND(A=1, A=2) returns the same since it produces a contradiction', function (assert) {
        ...
        assert.ok(filterSimplify.isContradicted(), "then a contradiction is found");
    });
    ...
    QUnit.test('applyStartFilter: reduce OR(A=1,A=2,A=1,A=4) by OR(A=1,A=2,C=42)', function (assert) {
        ...
        var filterSimplify = new sap.apf.core.utils.FilterReduction();
        var transforms = filterSimplify.transformFilter([disj]);
        var simplifiedTransforms = filterSimplify.simplifyTransforms(transforms);
        var result = filterSimplify.applyStartFilter(reducer, simplifiedTransforms);
    ...
    QUnit.test('filterReduction AND(AND(OR(eq, eq),empty), empty) - AND in left branch, returns the OR', function (assert) {
        var andNode = new sap.apf.core.utils.Filter(this.messageHandler);
        var orNode = new sap.apf.core.utils.Filter(this.messageHandler, 'A', this.EQ, "de")
            .addOr(new sap.apf.core.utils.Filter(this.messageHandler, 'A', this.EQ, "fr"));
        andNode.addAnd(orNode).addAnd(new sap.apf.core.utils.Filter(this.messageHandler));
        var filter = new sap.apf.core.utils.Filter(this.messageHandler);
        filter.addAnd(andNode).addAnd(new sap.apf.core.utils.Filter(this.messageHandler));
    ...
    QUnit.test('filterReduction AND(empty, AND(OR(eq, eq),empty)) - AND spine is standard case in path, returns the OR', function (assert) {
        ...
        andNode.addAnd(orNode).addAnd(new sap.apf.core.utils.Filter(this.messageHandler));
        var filter = new sap.apf.core.utils.Filter(this.messageHandler);
        filter.addAnd(new sap.apf.core.utils.Filter(this.messageHandler)).addAnd(andNode);
    ...
    QUnit.test('filterReduction AND(empty, OR(1,2,1)) - returns the OR', function (assert) {
        ...
        var filter = new sap.apf.core.utils.Filter(mh);
    ...
    QUnit.test('filterReduction AND(OR(A=1,A=2),B=1), OR(A=1,A=2),C=42))', function (assert) {
        ...
        var filter = new sap.apf.core.utils.Filter(this.messageHandler, leftOr);
        filter.addAnd(rightOr);

        var filterSimplify = new sap.apf.core.utils.FilterReduction();
        var reducedFilter = filterSimplify.filterReduction(mh, filter);
