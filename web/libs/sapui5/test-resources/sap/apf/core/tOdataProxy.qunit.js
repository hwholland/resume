...
(function() {
	...
	function commonSetupOdataProxy(assert, context, statusCode, statusText) {
		...
		var coreApi = {
			...
			getEntityTypeMetadata : function(sServiceRoot, entityType) {
				thatAssert.equal(sServiceRoot, "/serviceRoot", "expected service root");
				return {
					meta : "data",
	...
	QUnit.test("Read all Applications synchronously", function(assert) {
		assert.expect(5);
		this.odataProxy.readCollection("application", this.assertSuccessOnReadCollection, undefined, undefined, undefined, false);
		assert.equal(this.request.requestUri, "/serviceRoot/ApplicationQueryResults?$orderby=ApplicationName", "THEN correct url for batch");
	});
	...
	QUnit.test("Read all Configurations with restriction to selected fields synchronously", function(assert) {
		assert.expect(5);
		this.odataProxy.readCollection("configuration", this.assertSuccessOnReadCollection, undefined, [ "AnalyticalConfiguration", "AnalyticalConfigurationName", "Application", "CreatedByUser", "CreationUTCDateTime", "LastChangeUTCDateTime",
				"LastChangedByUser" ], undefined, false);
		assert.equal(this.request.requestUri, "/serviceRoot/AnalyticalConfigurationQueryResults?$select=AnalyticalConfiguration,AnalyticalConfigurationName,Application,CreatedByUser,CreationUTCDateTime,LastChangeUTCDateTime,LastChangedByUser",
				"correct url");
	});
	QUnit
			.test(
					"Read all Configurations of an Application",
					function(assert) {
						assert.expect(6);
						...
						assert
								.equal(
										this.request.data.__batchRequests[0].requestUri,
	...
	QUnit.test("Read operations send synchronously", function(assert) {
		this.odataProxy.readCollection("application", function() {
		});
		assert.equal(this.request.async, true, "If readCollection is called without async parameter, request is send asynchronously");
		this.odataProxy.readCollection("application", function() {
		}, undefined, undefined, undefined, false);
		assert.equal(this.request.async, false, "If readCollection is called with async parameter false, request is send synchronously");
		this.odataProxy.readEntity("application", function() {
		}, undefined, undefined);
		assert.equal(this.request.async, true, "If readEntity is called without async parameter, request is send asynchronously");
		this.odataProxy.readEntity("application", function() {
		}, undefined, undefined, false);
		assert.equal(this.request.async, false, "If readEntity is called with async parameter false, request is send synchronously");
	});
	QUnit.module("M: Create operations of OdataProxy", {
		beforeEach : function(assert) {
			commonSetupOdataProxy(assert, this, 201, "Created");
		}
	});
	...
	QUnit.module("M: Delete operations of OdataProxy", {
		beforeEach : function(assert) {
			commonSetupOdataProxy(assert, this, 201, "No Content");
		}
	});
	...
	QUnit.module("M: Error handling of OdataProxy", {
		beforeEach : function(assert) {
			...
			var coreApi = {
				...
				getEntityTypeMetadata : function(sServiceRoot, entityType) {
					assert.equal(sServiceRoot, "/serviceRoot", "expected service root");
					assert.equal(entityType, 'ApplicationQueryResults', 'expected entity type');
					return {
						meta : "data"
					};
				},
	...
	QUnit.test("M: Read collections synchronously", function(assert) {
		this.odataProxy.readCollectionsInBatch([], function() {
		});
		assert.equal(this.request.async, true, "If readEntity is called without async parameter, request is send asynchronously");
		this.odataProxy.readCollectionsInBatch([], function() {
		}, false);
