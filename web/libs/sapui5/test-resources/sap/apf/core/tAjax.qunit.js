...
(function() {
	...
	QUnit.test('GIVEN an AJAX stub simulating a NON existing resource WHEN send simple request THEN error function are called', function(assert) {
		...
		sap.apf.core.ajax(this.ajaxInject); //<<< unit under test
		assert.ok(spyAJAX.calledOnce);
		assert.ok(this.bRequestError, "Request successfully sent and success function was called");
		spyAJAX.restore();
	});
	...
	QUnit.test('Timout with redirect to login 200', function(assert) {
		var bRequestError = false;
		this.server = sinon.fakeServer.create();
