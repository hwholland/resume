jQuery.sap.declare('sap.apf.testhelper.odata.sampleServiceData');
jQuery.sap.declare('sap.apf.testhelper.odata.injectODataDouble');

sap.apf.testhelper.odata.injectODataDouble = function() {
	var sUri = '';
	...
	var datajs = {
		request : function(oRequest, fnSuccess) {
			...
			if (entityType) {
				...
				data.__batchResponses[0].data.results = sap.apf.testhelper.odata.getSampleServiceData(entityType).data;
				fnSuccess(data);
			}
		}
	};
	sap.apf.testhelper.odata.getUriFromODataSpy = function() {
		return sUri;
	};
...
sap.apf.testhelper.odata.getSampleServiceData = function(sEntityType) {
	var aReturn = [];
	var addRow = function() {
		aReturn.push({
