...
(function() {
	...
	function commonSetupUtilsFilter(oContext) {
		var oMsgHandler = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging();
		oContext.oMsgHandler = oMsgHandler;
		oContext.oFilter = new sap.apf.utils.Filter(oMsgHandler);
	...
	QUnit.test("getOperators", function(assert) {
		...
		assert.equal(eq, sap.apf.core.constants.FilterOperators.EQ, "correct operator returned");
		var oOperators = this.oFilter.getOperators();
		assert.equal(oOperators.LE, sap.apf.core.constants.FilterOperators.LE, "correct operator returned");
	});
	...
	QUnit.test("Method getInternalFilter for EXPRESSION on AND", function(assert) {
		...
		var oCompareFilter = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.EQ, "country1");
	...
	QUnit.test("Method getInternalFilter for EXPRESSION StartsWith on OR", function(assert) {
		...
		this.oFilter.getTopAnd().addOr('orId').addExpression(oExpression);
		var oInternalFilter = this.oFilter.getInternalFilter();
		var oCompareFilter = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.StartsWith, "cou");
		assert.ok(oInternalFilter.isEqual(oCompareFilter), "Function getInternalFilter returns expected filter");
	});
	QUnit.test("Method getInternalFilter for EXPRESSION Between on OR", function(assert) {
		...
		this.oFilter.getTopAnd().addOr('orId').addExpression(oExpression);
		var oInternalFilter = this.oFilter.getInternalFilter();
		var oCompareFilter = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.BT, "cou", "dou");
		assert.ok(oInternalFilter.isEqual(oCompareFilter), "Function getInternalFilter returns expected filter");
	});
	...
	QUnit.test("Method getTopAnd", function(assert) {
		assert.equal(this.oFilter.getTopAnd(), this.oFilter.getById(sap.apf.utils.Filter.topAndId), "Top 'AND' returned");
	});
	QUnit.test("Combined expressions of AND and OR", function(assert) {
		...
		var oInternalFilterCountry = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.EQ, "BRA");
		var oInternalFilterCustomer1 = new sap.apf.core.utils.Filter(this.oMsgHandler, "customer", this.EQ, "customer1");
		var oInternalFilterCustomer2 = new sap.apf.core.utils.Filter(this.oMsgHandler, "customer", this.EQ, "customer2");
		var oInternalFilterCustomer = new sap.apf.core.utils.Filter(this.oMsgHandler, oInternalFilterCustomer1.addOr(oInternalFilterCustomer2));
		var oInternalFilter = new sap.apf.core.utils.Filter(this.oMsgHandler, oInternalFilterCountry).addAnd(oInternalFilterCustomer);
	...
	QUnit.test("Create simple compound filter", function(assert) {
		...
		var oInternalFilterCountry = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.EQ, "country1");
		var oInternalFilterCity = new sap.apf.core.utils.Filter(this.oMsgHandler, "city", this.EQ, "city1");
	...
	QUnit.test("Create complex compound filter", function(assert) {
		...
		assert.deepEqual(aExpressions, aExpectedExpressions, "getExpressions() returns correct compound filter");
		var oInternalFilterCompany = new sap.apf.core.utils.Filter(this.oMsgHandler, "company", this.EQ, "company1");
		var oInternalFilterCustomer = new sap.apf.core.utils.Filter(this.oMsgHandler, "customer", this.EQ, "customer1");
		var oInternalFilterCountry1 = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.EQ, "country1");
		var oInternalFilterCountry2 = new sap.apf.core.utils.Filter(this.oMsgHandler, "country", this.EQ, "country2");
		var oInternalFilterCity1 = new sap.apf.core.utils.Filter(this.oMsgHandler, "city", this.EQ, "city1");
		var oInternalFilterCity2 = new sap.apf.core.utils.Filter(this.oMsgHandler, "city", this.EQ, "city2");
	...
	QUnit.test("Deserialize filter", function(assert) {
		...
		var oNewFilter = new sap.apf.utils.Filter(this.oMsgHandler);
	...
	QUnit.test("Intersection has right and unique IDs", function(assert) {
		...
		var oMsgHandler = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging();
		var oSecondFilter = new sap.apf.utils.Filter(oMsgHandler);
		...
		assert.ok(oResultFilter.getTopAnd().getId() === sap.apf.utils.Filter.topAndId, "Result filter has the right ID for the topAnd expression");
		var aIds = {};
		var j = 0;
	...
	QUnit.test("Intersection has right value", function(assert) {
		...
		oSecondFilter.getById("orId1").addAnd("andId1").addExpression(oExprCountry1b).addExpression(oExprCity1b);
		oSecondFilter.getById("orId1").addAnd("andId2").addExpression(oExprCountry2b).addExpression(oExprCity2b);
		var oExpectedFilter = new sap.apf.utils.Filter(oMsgHandler);
		...
		oResultFilter = new sap.apf.utils.Filter(oMsgHandler);
	...
	QUnit.test("WHEN createFilterFromSapUi5FilterJSON", function(assert) {
		...
		function createOriginalFilter() {
			var CostingAreaFilter1 = new sap.apf.core.utils.Filter(that.oMsgHandler, "CoArea", sap.ui.model.FilterOperator.EQ, "1000");
			var CostCenterFilter1 = new sap.apf.core.utils.Filter(that.oMsgHandler, "CostCenter", sap.ui.model.FilterOperator.EQ, "11000");
			var CostCenterCompoundFilter1 = CostCenterFilter1.addAnd(CostingAreaFilter1);
			var CostingAreaFilter2 = new sap.apf.core.utils.Filter(that.oMsgHandler, "CoArea", sap.ui.model.FilterOperator.EQ, "2000");
			var CostCenterFilter2 = new sap.apf.core.utils.Filter(that.oMsgHandler, "CostCenter", sap.ui.model.FilterOperator.EQ, "22000");
			var CostCenterCompoundFilter2 = CostingAreaFilter2.addAnd(CostCenterFilter2);
			var AllCostCenterCompoundFilter = new sap.apf.core.utils.Filter(that.oMsgHandler, CostCenterCompoundFilter1).addOr(CostCenterCompoundFilter2);
			var CompanyCodeFilter = new sap.apf.core.utils.Filter(that.oMsgHandler, "CompanyCode", sap.ui.model.FilterOperator.EQ, "1000");
			var YearFilter = new sap.apf.core.utils.Filter(that.oMsgHandler, "Year", sap.ui.model.FilterOperator.EQ, 2014);
			var topFilter = new sap.apf.core.utils.Filter(that.oMsgHandler, AllCostCenterCompoundFilter).addAnd(CompanyCodeFilter);
			topFilter = topFilter.addAnd(YearFilter);
			return topFilter;
		}
		var filter = createOriginalFilter();
		var jsonFormat = filter.mapToSapUI5FilterExpression();
		var filterFromTransformation = sap.apf.utils.Filter.createFilterFromSapUi5FilterJSON(that.oMsgHandler, jsonFormat).getInternalFilter();
