...
(function() {
	...
	function commonSetup(testEnv, deferCallback) {
		testEnv.msgH = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging();
	...
	QUnit.test('Get property name', function(assert) {
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpAndResolutionRequest);
	...
	QUnit.test('Get alias if defined', function(assert) {
		var startFilterOne = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpRequest);
		var startFilterTwo = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpAndResolutionRequest);
	...
	QUnit.test('Get selection mode', function(assert) {
		var startFilterSingleSelection = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpRequest);
		var startFilterMultiSelection = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpAndResolutionRequest);
	...
	QUnit.test('Instantiate filter marked as invisible', function(assert) {
		var visibleStartFilter = new sap.apf.utils.StartFilter(this.inject, {});
		var invisibleStartFilter = new sap.apf.utils.StartFilter(this.inject, {
			invisible : true
		});
	...
	QUnit.test('Instantiate filter that implicitely determines that it is invisible', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.inject, {}, filter);
	...
	QUnit.test('Correct property name requested if no alias defined', function(assert) {
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpAndResolutionRequest);
		startFilter.getMetadata();
		assert.ok(this.getPropertyMetadataSpy.calledWith('PropertyTwo'), 'Metadata retrieved for correct property');
	});
	QUnit.test('Correct property name requested if alias defined', function(assert) {
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest);
		startFilter.getMetadata();
		assert.ok(this.getPropertyMetadataSpy.calledWith('AliasForPropertyOne'), 'Metadata retrieved for correct property');
	});
	QUnit.test('Property specific metadata resolved after request response', function(assert) {
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest);
		assert.expect(1);
		startFilter.getMetadata().done(function(result) {
			assert.deepEqual(result, {
				label : 'metadataValueHelpRequest',
				filterable : false
			}, 'Correct metadata for property returned');
		});
	});
	QUnit.test('No request is defined', function(assert) {
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithoutRequest);
		assert.expect(1);
		startFilter.getMetadata().done(function(result) {
			assert.deepEqual(result, {}, 'Empty metadata object returned');
		});
	});
	QUnit.test('Only filter resolution request is defined', function(assert) {
		var stubFilter = new sap.apf.core.utils.Filter(this.msgH);
		stubFilter.stubControl = 'filterResolution';
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithResolutionRequestOnly, stubFilter );
		assert.expect(1);
		startFilter.getMetadata().done(function(result) {
			assert.deepEqual(result, {
				label : 'metadataFilterResolutionRequest',
				filterable : false
			}, 'Metadata from filter resolution request returned');
		});
	});
	QUnit.test('Only filter resolution request is defined with no context', function(assert) {
		this.requestType = 'filterResolution';
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithResolutionRequestOnly, undefined);
		assert.expect(1);
		startFilter.getMetadata().done(function(result) {
			assert.deepEqual(result, {
				label : 'metadataFilterResolutionRequest',
	...
	QUnit.module('Get list values', {
		beforeEach : function() {
			commonSetup(this);
			this.startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest);
			this.filterForProperty = new sap.apf.core.utils.Filter(this.msgH);
	...
	QUnit.test('When preselection defaults are configured for single selection only', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigSinglePreselectionOnly, undefined);
	...
	QUnit.test('When preselection defaults are configured for multi selection only', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigMultiPreselectionOnly, undefined);
		startFilter.getValues().done(function(result) {
			assert.deepEqual(result, [ {
				PropertySix : 'value1'
			}, {
	...
	QUnit.test('When external context is list and only filter resolution request configured', function(assert) {
		assert.expect(2);
		var testEnv = this;
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithResolutionRequestOnly, [ 'valueX', 'valueY', 'valueZ' ]);
	...
	QUnit.test('When external context is list, valueList is an empty array and only filter resolution request configured', function(assert) {
		...
		config.valueList = []; 
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, config, [ 'valueX', 'valueY', 'valueZ' ]);
		startFilter.getValues().done(
				function(result) {
					assert.equal(testEnv.createRequestSpy.getCall(0).args[0], 'FRRPropertyFour', 'Filter resolution request for correct id created');
					assert.equal(testEnv.sendGetInBatchSpy.getCall(0).args[0].toUrlParam(), '((PropertyFour%20eq%20%27valueX%27)%20or%20(PropertyFour%20eq%20%27valueY%27)%20or%20(PropertyFour%20eq%20%27valueZ%27))',
	...
	QUnit.test('When external context is list and no request configured', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithoutRequest, [ 'valueX', 'valueY', 'valueZ' ]);
		startFilter.getValues().done(function(result) {
			assert.deepEqual(result, [ {
	...
	QUnit.test('When no external context but valueList', function (assert){
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpDefaults, undefined);
		startFilter.getValues().done(function(result){
			assert.deepEqual(result, [{
	...
	QUnit.test('When no intersection between external context as list and value help request', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest, [ 'valueD', 'valueE', 'valueF' ]);
		startFilter.getValues().done(function(result) {
	...
	QUnit.test('When external context list intersects with value help request', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest, [ 'valueA', 'valueC', 'valueD' ]);
		startFilter.getValues().done(function(result) {
	...
	QUnit.test('When external context is filter expression and there is no filter resolution request defined', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithoutRequest, this.filterForProperty);
		startFilter.getValues().done(function(result) {
	...
	QUnit.test('When external context is filter expression and only filter resolution request is defined', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithResolutionRequestOnly, this.filterForProperty);
		startFilter.getValues().done(function(result) {
			assert.ok(testEnv.sendGetInBatchSpy.getCall(0).args[0].isEmpty(),'sendGetInBatch is called with empty filter');
			assert.deepEqual(result, [ {
				PropertyFour : 'valueE',
	...
	QUnit.test('When no intersection between result of external context as filter expression with filter resolution request and value help request', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpAndResolutionRequest, this.filterForProperty);
		startFilter.getValues().done(function(result) {
	...
	QUnit.test('Without external context, value help request and filter resolution request are not defined', function(assert) {
		assert.expect(2);
		var testEnv = this;
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithoutRequest, undefined);
		startFilter.getValues().done(function(result) {
			assert.equal(testEnv.createRequestSpy.called, false, 'No request triggered');
			assert.ok(result === null, 'Null returned because values cannot be determined');
		});
	...
	QUnit.test('When NO value help and preselection function', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigPreselectionFunctionWithoutIntersection, undefined);
		startFilter.getValues().done(function(result) {
			assert.deepEqual(result, [ {
	...
	QUnit.test('When value help and preselection function', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigPreselectionFunctionWithIntersection, undefined);
		startFilter.getValues().done(function(result) {
			assert.deepEqual(result, [ {
	...
	QUnit.test('When filter resoltion request and preselection defaults', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigPreselectionDefaultsFilterResolution, undefined);
		startFilter.getValues().done(function(result) {
			assert.deepEqual(result, [ {
	...
	QUnit.test('When only preselection defaults as list are configured for single selection', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigSinglePreselectionOnly);
	...
	QUnit.test('When only preselection defaults as list are configured for multi selection', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigMultiPreselectionOnly);
		startFilter.getSelectedValues().done(function(result) {
	...
	QUnit.test('When preselection defaults configured and value help request is defined for multi selection', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpAndResolutionRequest, undefined);
		startFilter.getSelectedValues().done(function(result) {
			assert.equal(testEnv.createRequestSpy.getCall(0).args[0], 'VHRPropertyTwo', 'Value help request for correct id created');
			assert.ok(testEnv.sendGetInBatchSpy.getCall(0).args[0].isEmpty(), 'Value help sendGetInBatch is called with empty Filter');
	...
	QUnit.test('When preselection defaults configured and value help request is defined for single selection', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfig, undefined);
		startFilter.getSelectedValues().done(function(result) {
			assert.equal(testEnv.createRequestSpy.getCall(0).args[0], 'VHRPropertyTwo', 'Value help request for correct id created');
			assert.ok(testEnv.sendGetInBatchSpy.getCall(0).args[0].isEmpty(), 'Value help sendGetInBatch is called with empty Filter');
	...
	QUnit.test('When external context is list, single selection and no request configured', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithoutRequest, [ 'valueA', 'valueB', 'valueC' ]);
		startFilter.getSelectedValues().done(function(result) {
	...
	QUnit.test('When external context is list, multi selection and no request configured', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithoutRequestMulti, [ 'valueA', 'valueB', 'valueC' ]);
		startFilter.getSelectedValues().done(function(result) {
	...
	QUnit.test('When external context is filter expression and no request configured', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithoutRequest, this.filterForProperty);
		startFilter.getSelectedValues().done(function(result) {
	...
	QUnit.test('When external context is filter expression and filter resolution request is defined for multi selection', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithResolutionRequestOnlyMulti, this.filterForProperty);
		startFilter.getSelectedValues().done(function(result) {
			assert.equal(testEnv.createRequestSpy.getCall(0).args[0], 'FRRPropertyFour', 'Filter resolution request for correct id created');
			assert.ok(testEnv.sendGetInBatchSpy.getCall(0).args[0].isEmpty(), 'Filter resolution sendGetInBatch is called with empty filter');
	...
	QUnit.test('When only value help request is defined for single selection', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest, undefined);
		startFilter.getSelectedValues().done(function(result) {
	...
	QUnit.test('When only value help request is defined for multi selection', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequestMulti, undefined);
		startFilter.getSelectedValues().done(function(result) {
			assert.equal(testEnv.createRequestSpy.getCall(0).args[0], 'VHRPropertyOne', 'Value help request for correct id created');
			assert.ok(testEnv.sendGetInBatchSpy.getCall(0).args[0].isEmpty(), 'Value help sendGetInBatch is called with empty Filter');
	...
	QUnit.test('When preselectionDefaults are partly restricted by dependent startfilter', function(assert){
		...
		startFilter.getSelectedValues().done(function(result, promise){
			...
			startFilter.setRestriction(new sap.apf.core.utils.Filter(testEnv.msgH, 'Test', 'eq', 'TestValue'));
	...
	QUnit.test('When all selected values are restricted by dependent startfilter', function(assert){
		var testEnv = this;
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpAndResolutionRequest, undefined);
		var valueHelpBeforeRestriction =  [ {
					PropertyTwo : 'value1'
	...
	QUnit.test('Set filter as selected values returns empty array if start filter is visible', function(assert) {
		assert.expect(1);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithoutRequest);
		startFilter.setSelectedValues(new sap.apf.core.utils.Filter(this.msgH));
	...
	QUnit.test('Set filter value on a startFilter with value list ', function(assert) {
		assert.expect(2);
		var startFilter = new sap.apf.utils.StartFilter(this.inject, filterConfigWithValueHelpDefaults);
	...
	QUnit.test('Apply restriction on list values with external context values', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest, [ 'containedInValueHelp', 'notContainedInValueHelp' ]);
		...
		startFilter.getValues().done(function(values, promise) {
			...
			startFilter.setRestriction(new sap.apf.core.utils.Filter(this.msgH, 'PropertyTest', 'eq', 'Test'));
			...
			promise.done(function(values, promise){
				...
				startFilter.setRestriction(new sap.apf.core.utils.Filter(this.msgH));
	...
	QUnit.test('Apply restriction on getSelectedValues with external context values', function(assert) {
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, config, [ 'containedInValueHelp', 'notContainedInValueHelp' ]);
	...
	QUnit.test('Value help request', function(assert) {
		...
		var done = assert.async();
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest);
		var restriction = new sap.apf.core.utils.Filter(this.msgH, 'RestrictiveProperty', 'eq', 'RestrictionValue');
	...
	QUnit.test('Copy of filter restriction used in Value help request', function(assert) {
		...
		var done = assert.async();
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest);
		var restriction = new sap.apf.core.utils.Filter(this.msgH);
		startFilter.setRestriction(restriction);
	...
	QUnit.test('Filter resolution request', function(assert) {
		assert.expect(1);
		var done = assert.async();
		var filterForProperty = new sap.apf.core.utils.Filter(this.msgH, 'PropertyFour', 'lt', '100');
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithResolutionRequestOnly, filterForProperty);
		var restrictiveFilter = new sap.apf.core.utils.Filter(this.msgH, 'RestrictiveProperty', 'eq', 'RestrictionValue');
	...
	QUnit.test('When different restrictions are set subsequently promise only is resolved with last restriction', function(assert) {
		...
		var filterForProperty = new sap.apf.core.utils.Filter(this.msgH);
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpAndResolutionRequest, filterForProperty);
		startFilter.getSelectedValues().done(function(values) {
			assert.deepEqual(values, ['valueE'], 'Correct value from last request invocation being triggered by 2nd setRestriction() returned');
			done();
		});
		startFilter.setRestriction(new sap.apf.core.utils.Filter(this.msgH, 'PropertyTest', 'eq', 'Test'));
		startFilter.setRestriction(new sap.apf.core.utils.Filter(this.msgH, 'PropertyTest', 'eq', 'Test2'));

		setTimeout(function() {this.callbacksSendGetInBatch[0](A);}.bind(this), 8);
		setTimeout(function() {this.callbacksSendGetInBatch[1](B);}.bind(this), 1);
		setTimeout(function() {this.callbacksSendGetInBatch[2](C);}.bind(this), 16);
		setTimeout(function() {this.callbacksSendGetInBatch[3](D);}.bind(this), 8);
		setTimeout(function() {this.callbacksSendGetInBatch[4](E);}.bind(this), 4);
	});
	QUnit.test('When different restrictions are set subsequently only promise for getValues from last restriction is resolved', function(assert) {
		...
		startFilter.getValues().done(function(values, promise) {
			...
			startFilter.setRestriction(new sap.apf.core.utils.Filter(testEnv.msgH, 'PropertyTest', 'eq', 'Test2'));
	...
	QUnit.test('When different restrictions are set subsequently only promise for getSelectedvalues from last restriction is resolved', function(assert) {
		...
		config.multiSelection = true;
		var filterForProperty = new sap.apf.core.utils.Filter(this.msgH);
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, config, filterForProperty);
	...
	QUnit.test('Serialize', function(assert) {
		var isNavigation;
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigMultiPreselectionOnly);
	...
	QUnit.test('Serialize when selected values are "null"', function(assert) {
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithoutRequest);
...
//			assert.deepEqual(selectedValues, ['value3'], 'Values reset to state from deserialization');
	...
	QUnit.test('SelectedValues is internal filter', function(assert) {
		...
		var filter = new sap.apf.core.utils.Filter(this.msgH, 'PropertyTwo', 'lt', 'value2');
	...
	QUnit.test('Avoid call by reference in value help while buffering requests when prepending context', function (assert){
		...
		var startFilter = new sap.apf.utils.StartFilter(this.injectCreateRequestSpy, filterConfigWithValueHelpRequest, ['someValue', '1000', '2000']);
		startFilter.getValues().done(function (values, promise){
			...
			deferredSerialize.done(function(serializedStartFilter){
				...
				startFilter.getValues().done(function(values, promise){
					startFilter.setRestriction(new sap.apf.core.utils.Filter(testEnv.msgH, 'A', 'eq' ,'1'));
