...
(function() {
	...
	QUnit.test("serializeConfiguration with complete smartFilterbar", function(assert) {
		...
		assert.equal(serializedObject.smartFilterBar.entityType, entityType, "EntityType from SmartFilterBar retrieved");
	});
	QUnit.test("serializeConfiguration with empty smartFilterbar", function(assert) {
		...
		assert.equal(serializedObject.smartFilterBar.entityType, undefined, "Undefined if empty SmartFilterBar");
	});
	...
	QUnit.test("serializeConfiguration representation", function(assert) {
		...
		assert.equal(obj.bindings[0].representations[0].parameter.measures[0].fieldName, 'OneMeasurePropOne', '1st measue of 1st representation');
	...
	QUnit.module("Map to design time", {
		beforeEach : function() {
			...
			this.produceACompleteStep = function(producer, categoryId) {
			...
			this.produceACompleteConfiguration = function(producer) {
			...
			this.configurationEditor.registerService = function(service) {
				return null;
			};
			this.registry = new RegistryProducer();
		}
	});
	QUnit.test("Application title", function(assert) {
		...
		this.configurationObjects.mapToDesignTime(this.registry, this.configurationEditor);
	...
	QUnit.test("GIVEN registry with step", function(assert) {
		var that = this;
		...
		this.registry.executeTheSerialization();
		this.configurationEditor.registerService = function(service) {
			assert.equal(service, that.service13, "WHEN mapToDesignTime THEN registerService is called with the serialized service");
		};
		this.configurationObjects.mapToDesignTime(this.registry, this.configurationEditor);
		...
		assert.equal(this.configurationEditor.getSteps()[0].getFilterProperties()[0], this.propertyFilter410, "WHEN mapToDesignTime THEN binding exists in editor");
	...
	QUnit.test('Deserialize representation for step', function(assert) {
		...
		assert.equal(modelRepresentation.getLabelDisplayOption('DimPropOne'), undefined, 'undefined displayOption for propertyOne');
		assert.equal(modelRepresentation.getLabelDisplayOption('DimPropTwo'), displayOptions.TEXT, 'Text as displayOption for propertyOne');
	...
	QUnit.test("Deserialize SmartFilterBar", function(assert) {
		...
		assert.equal(smartFilterBar.getEntityType(), entityType, "EntityType exists");
	...
	QUnit.test('Deserialize empty facet filter array', function(assert) {
		...
		this.configurationObjects.mapToDesignTime(registryWrapper, this.configurationEditor);
	...
	QUnit.test("Deserialize facet filter", function(assert) {
		...
		var that = this;
		this.configurationEditor.registerService = function(service) {
			assert.ok([ that.serviceForValueHelp, that.serviceForFilterResolution ].indexOf(service) >= 0, "WHEN mapToDesignTime THEN registerService is called with the serialized services");
		};
		this.configurationObjects.mapToDesignTime(this.registry, this.configurationEditor);
	...
	QUnit.module("Serializable configuration object", {
		beforeEach : function() {
			...
			this.labelKey11 = "labelKey-1100";
			this.preselectionFunction11 = "preselectionFunction11";
			this.produceACompleteFacetFilter = function(producer) {
				// facet filter setup
				producer.setFilterOption({facetFilter : true});
				var facetFilterId = producer.createFacetFilterWithId(that.facetFilterKey);
				var facetFilter = producer.getFacetFilter(facetFilterId);
			...
			this.produceACompleteStep = function(producer, categoryId) {
				var stepId = producer.createStepWithId(that.stepKey320, categoryId);
				var step = producer.getStep(stepId);
				step.setTitleId(that.titleId42);
				step.setLongTitleId(that.longTitleId43);
				step.setLeftLowerCornerTextKey('setLeftLowerCornerTextKey');
	...
	QUnit.test("GIVEN a serializable configuration object", function(assert) {
		assert.expect(1);
		var expectedResult = [ "titleId42", "longTitleId43", "setLeftUpperCornerTextKey", "setLeftLowerCornerTextKey", "setRightUpperCornerTextKey", "setRightLowerCornerTextKey", "LabelKeyForDimPropOne", "LabelKeyForMeasurePropOne",
				"setLeftUpperCornerTextKey", "setLeftLowerCornerTextKey", "setRightUpperCornerTextKey", "setRightLowerCornerTextKey", "textKey-4711", "labelKey-1100" ];
