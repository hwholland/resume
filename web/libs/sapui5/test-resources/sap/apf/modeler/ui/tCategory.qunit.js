...
(function() {
	'use strict';
	var oModelerInstance, categoryView, getCategoryStepAssignmentsSpy, oTextPool, spyOnGetOfTextPool, spyOnSetTextOfTextPool, spyOnAutoComplete, spyOnUpdateSelectedNode, setCategorySpy, setIsUnsavedSpy;
	...
	QUnit.test("When Category view is loaded", function(assert) {
		...
		var oInputControl = categoryView.byId("idCategoryTitle");
		...
		assert.strictEqual(spyOnAutoComplete.calledOnce, true, "then autocomplete function is called once for category view");
		assert.strictEqual(spyOnAutoComplete.calledWith(oInputControl, oDependenciesForText), true, "then autocomplete function is called with correct arguments for category view");
		...
		assert.deepEqual(document.body, focusedElement, "Since the category is already saved with title " + 
				"so body has focus instead category title");
		...
		assert.strictEqual(categoryView.byId("idTotalSteps").getValue(), "2", "then, Number of steps is set for unsaved category");
		document.body.removeChild(document.getElementById('contentOfCategory'));
	});
	QUnit.test("When handleChangeDetailValue called while editing the category title for existing category", function(assert) {
		// arrange
	...
	QUnit.test("When handleChangeDetailValue called while emptying the category title for existing category", function(assert) {
		// arrange
		var oTranslationFormat = sap.apf.modeler.ui.utils.TranslationFormatMap.CATEGORY_TITLE;
		// act
	...
	QUnit.test("When Category view is loaded", function(assert) {
		...
		assert.strictEqual(categoryView.byId("idCategoryTitle").sId, focusedElement.parentElement.id, "Since the category is " + 
				"new so category title has focus on it ");
	...
	QUnit.test("When handleChangeDetailValue called while editing the category title for new category", function(assert) {
		// arrange
