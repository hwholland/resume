...
asyncTest("Read structured saved paths", function() {
	expect(1);
	var oRequest = {
		method : "GET",
	};
	var fnSuccess = function(oData, oResponse) {
		if (oData && oData.results && oData.results.length > 0 && oResponse.statusCode === 200) {
			ok(true, "Request responsed with more then 0 paths.");
		} else {
			ok(false, "Request responsed with more then 0 paths.");
		}
		start();
	};
...
asyncTest("Structured saved paths in descending modification order", function() {
	expect(2);
	var bOk = false;
	function assertDescendingOrder(oData) {
		...
		ok(bOk, 'Following change timesstamp must be smaller.');
	}
	var oRequest = {
		method : "GET",
	};
...
asyncTest("Read serialized saved path with invalid ID", function() {
	...
	var fnError = function(oError) {
		...
		if (oError.response.statusText === "Not Found" && oError.response.statusCode === 404 && oErrorResponseBody.error.message.value === "Resource not found.") {
			...
			start();
		} else if (oError.response.statusCode === 400 && config.systemType === "abap") {
			ok(true, "Request responded resource not found.");
			start();
		} else {
...
asyncTest("Read serialized saved path", function() {
	...
	var oRequest = {
		...
		success : function(oData, sStatus, oXMLHttpRequest) {
			ok(oData.path.AnalysisPath.length > 0, "Request responsed with exactly one path.");
			start();
		}.bind(this)
	};
...
//asyncTest("Delete all except ten", function() {
//	    expect(0);
