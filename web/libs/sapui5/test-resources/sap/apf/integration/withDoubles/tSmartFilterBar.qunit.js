(function() {
	...
	sap.ui.controller("sap.test.controller.smartFilterBar", {
		onInit : function() {
			...
			oMockServer.start();
			var oModel = new sap.ui.model.odata.ODataModel("/foo", true);
	...
	function commonSetup(context) {
		...
		var ExternalContext = function () {
			this.getCombinedContext = function () {
				return jQuery.Deferred().resolve(context.combinedContextFilter);
			};
		};
		var inject = {
				constructors : {
		...
		sap.apf.testhelper.createComponent(context, 
				{ stubAjaxForResourcePaths : true, componentId : "CompId1", path : "pathOfNoInterest",  inject : inject, componentData : {}});
		context.callbackForUpdatePath = function(oStep) {
			return null;
		};
		context.oComponent.getProbe().coreApi.loadAnalyticalConfiguration(sap.apf.testhelper.config.getSampleConfiguration('addSmartFilterBar'));
		context.createSFBview = function(){
			context.SFBview = sap.ui.view({
				...
				viewData : {
					...
					oSmartFilterBarConfiguration : context.oComponent.getProbe().coreApi.getSmartFilterBarConfiguration()
				}
			});
		};
	...
	QUnit.test('External context not applicable for SmartFilterbar', function(assert) {
		...
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler, "PropertyA", "EQ", "1");
		this.createSFBview();

		this.oComponent.getProbe().coreApi.createStep("stepTemplateComponent1", function(){
			jQuery.when(that.oComponent.getProbe().coreApi.getCumulativeFilter(), that.oComponent.getProbe().coreApi.getSmartFilterBar()).done(function(cumulativeStartFilter, smartFilterBarInstance){
	...
	QUnit.test('External context fully applicable for SmartFilterbar', function(assert) {
		...
		var done = assert.async();
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler, "Company", "EQ", "Company1");
	...
	QUnit.test('External context fully applicable for SmartFilterbar with ranges for not restricted filter properties', function(assert) {
		...
		var done = assert.async();
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler, "Country", "GT", "Argentina");
	...
	QUnit.test('Empty external context', function(assert) {
		...
		var done = assert.async();
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler);
		this.createSFBview();
	...
	QUnit.test('Multi value filter for single value property', function(assert) {
		...
		var done = assert.async();
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler, "City", "EQ", "Walldorf");
	...
	QUnit.test('Range filter for single value property', function(assert) {
		...
		var done = assert.async();
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler, "City", "GT", "Walldorf");
		this.createSFBview();
	...
	QUnit.test('Filter for a property that only restricts a value help', function(assert) {
		...
		var done = assert.async();
		var that = this;
		this.combinedContextFilter = new sap.apf.core.utils.Filter(this.messageHandler, "Country", "EQ", "Germany");
