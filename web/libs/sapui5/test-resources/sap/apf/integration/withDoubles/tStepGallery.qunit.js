(function() {
	...
	var arrangement = sap.ui.test.Opa5.extend("arrangement", {
		globalSetup : function() {
			...
			oGlobalApi = new sap.apf.testhelper.doubles.UiApi(undefined, "/apf-test/test-resources/sap/apf/testhelper/config/applicationConfigurationIntegration.json", inject);
	...
	var action = sap.ui.test.Opa5.extend("action", {
		representationClickStepGallery : function(oCurrentStep, bStepChanged) {
			oGlobalApi.oUiApi.getAnalysisPath().getCarousel().addButton.firePress();
			//sap.ui.getCore().byId(jQuery('.sapMLIB')[0].getAttribute("id")).firePress(); //FIXME To be removed after review
			//sap.ui.getCore().byId(jQuery('.sapMLIB')[1].getAttribute("id")).firePress(); //FIXME To be removed after review
		...
		checkStepContainer : function() {
			...
			return this.waitFor({
				...
				success : function() {
					...
					var selectedRepTypeThumbnailContent = oGlobalApi.oCoreApi.getSteps()[0].getSelectedRepresentation().getThumbnailContent().getContent()[0].getVizType();
