...
function eventText (oEvent) {
	...
	if (oEvent.getId() == "chartMouseOver") {
		...
		if (oParam.objectInfo !== undefined) {
			rowId = oParam.objectInfo.id;
		}else if (oParam.leadingObjectInfo !== undefined) {
			rowId = oParam.leadingObjectInfo.id;
		}else {
			rowId = "dummy row";
		}
	...
	createContent: function (oGanttChart) {
		var oRetVal = new sap.m.FlexBox({
			...
			items: [new sap.m.Label({
				...
				width: "200px",
				max: Math.log(oGanttChart._oZoom.determinedByConfig.fMaxRate),
				min: Math.log(oGanttChart._oZoom.determinedByConfig.fMinRate),
				value: Math.log(oGanttChart._oZoom.determinedByConfig.fRate),
...
sap.ui.jsfragment ( "sap.gantt.test.CSSModeButton",{ 
	createContent: function (oGanttChart ) {
		...
		var oModeSegmentedButton = new sap.m.SegmentedButton({
			buttons: [
				new sap.m.Button({text: "Compact", press: fnToggleCompactCozyMode('TOGGLE_COMPACT')}),
				new sap.m.Button({text: "Cozy", press: fnToggleCompactCozyMode('TOGGLE_COZY')}),
				new sap.m.Button({text: "Condense", press: fnToggleCompactCozyMode('TOGGLE_CONDENSED')}),
				new sap.m.Button({text: "Webdynpro", press: fnToggleCompactCozyMode('TOGGLE_WEBDYNPRO')})
			]
		});
