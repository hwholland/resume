...
(function () {
    "use strict";
    /*global jQuery, sap, strictEqual*/
    sap.ui.define(["sap/ui/test/Opa5", "sap/ui/test/matchers/Properties"],
        function (Opa5, Properties) {
        var mControls = {
                "button": { type: "sap.m.Button", idPrefix: "__button"},
                "item": { type: "sap.ui.core.Item", idPrefix: "__item"},
                "label": { type: "sap.m.Label", idPrefix: "__label"},
            ...
            mGenerics = {
                ...
                "sortItem": { matcher: undefined, defaultId: "scfld_SORT_ITEM",
                    control: mControls.item, bSearchOpenDialogs: true},
                "title": { matcher: undefined, control: mControls.label}
            },
                ...
                getCheckIdConfiguration: function (oInput) {
                    ...
                    return {
                        ...
                        success: function (aControls) {
                            var sId;
                            strictEqual(aControls.length, 1, "one Control - " + aControls[0].toString());
                            ...
                            if (!oInput.fixId && (oInput.isGenerated || !sExpectedId)) {
                                strictEqual(sId.slice(0, sIdPrefix.length), sIdPrefix,
                                    "Generated id with prefix: " + sIdPrefix);
                            } else {
                                strictEqual(sId, oInput.fixId || sExpectedId, oInput.scfldControl
                                    + " has expected id: " + (oInput.fixId || sExpectedId));
                            }
                        },
