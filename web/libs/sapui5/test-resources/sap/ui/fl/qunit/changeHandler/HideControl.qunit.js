...
(function(HideControlChangeHandler, Control, Element, Change, JsControlTreeModifier, XmlTreeModifier) {
	...
	QUnit.test("applyChange on a js control tree", function(assert) {
		...
		HideControlChangeHandler.applyChange(this.oChange, oControl, {modifier: JsControlTreeModifier});

		assert.equal(oControl.getVisible(), false);
	});


	QUnit.test("applyChange on a js control tree throws an exception if the change is not applyable", function(assert) {
		assert.throws(function () {
			var oElement = new Element();
			HideControlChangeHandler.applyChange(this.oChange, oElement, {modifier: JsControlTreeModifier});
	...
	QUnit.test("applyChange on a xml tree", function() {
		...
		HideControlChangeHandler.applyChange(this.oChange, this.oXmlButton, {
			modifier: XmlTreeModifier,
			view: this.oXmlDocument
		});

		HideControlChangeHandler.applyChange(this.oChange, this.oXmlButton, {modifier: XmlTreeModifier, view: this.oXmlDocument});
	...
	QUnit.test("completeChangeContent", function(assert) {
		...
		HideControlChangeHandler.completeChangeContent(this.oChange, oSpecificChangeInfo);
