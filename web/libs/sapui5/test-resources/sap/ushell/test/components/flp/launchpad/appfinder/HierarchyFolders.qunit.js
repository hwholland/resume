/**
 * @fileOverview QUnit tests for sap.ushell.components.flp.launchpad.appfinder.HierarchyFolders
 */
sap.ui.require([
    ...
    "use strict";
    /*global asyncTest, equal, expect, module, ok, start, stop, test, jQuery, sap, sinon */
    ...
    module("sap.ushell.components.flp.launchpad.appfinder.HierarchyFolders", {
        beforeEach: function () {
            sap.ushell.bootstrap("local");
            oController = new sap.ui.controller("sap.ushell.components.flp.launchpad.appfinder.HierarchyFolders");
    ...
    test("systemSelectorTextFormatter - with system selected with system name should call getText with system name ", function () {
        var spy = sinon.spy(sap.ushell.resources.i18n, "getText");
        var result = oController.systemSelectorTextFormatter({systemName: "name1", systemId: "id1"});
        assert.ok(spy.calledWith("easyAccessSelectSystemTextWithSystem","name1"));
        sap.ushell.resources.i18n.getText.restore();
    });

    test("systemSelectorTextFormatter - with system selected without system name should call getText with system id ", function () {
        var spy = sinon.spy(sap.ushell.resources.i18n, "getText");
        var result = oController.systemSelectorTextFormatter({systemName: undefined, systemId: "id1"});
        assert.ok(spy.calledWith("easyAccessSelectSystemTextWithSystem","id1"));
        sap.ushell.resources.i18n.getText.restore();
    });

    test("systemSelectorTextFormatter - without system selected should call getText with no system ", function () {
        var spy = sinon.spy(sap.ushell.resources.i18n, "getText");
        var result = oController.systemSelectorTextFormatter(undefined);
        assert.ok(spy.calledWith("easyAccessSelectSystemTextWithoutSystem"));
        sap.ushell.resources.i18n.getText.restore();
    });
     ...
     test("systemSearchHandler create a filter of systemName and filter of systemId", function () {
         ...
         var filtersSpy = sinon.spy(sap.ui.model,"Filter");
