...
(function () {
    ...
    function AppConfiguration() {
        ...
        this.addApplicationSettingsButtons = function (aButtons) {
             ...
             */
            if (oCurrentApplication && oCurrentApplication === oCurrentOpenedApplication){
        ...
        function addApplicationSettingsButtons(aButtons){
            var i,
                ...
                oComponentData = oRenderer && oRenderer.getComponentData ? oRenderer.getComponentData() : undefined,
                oConfig = oComponentData ? oComponentData.config : undefined;
            ...
            for (i = 0; i < aButtons.length; i++) {
                ...
                // add style class to action buttons in MeArea
                if (oConfig && oConfig.enableMeArea) {
                    oCurrentButton.addStyleClass('sapUshellActionItem');
         ...
         * @param sKey - the complete url hash of the application which consists of the app Intent and the parameters in lexicographically sorted order.
         * @private
         */
        this.addMetadata = function (oApplication, sKey) {
            try {
                ...
                if (sKey && !(oMetadata.hasOwnProperty(sKey))) {
                    oMetadata[sKey] = {}; //independent from application type - create an object for metadata

                    if (oComponentHandle) {
                        oLocalMetadataComponent = oComponentHandle.getMetadata();
                    } else if (sComponentName) {
                        jQuery.sap.log.warning("No component handle available for '" + sComponentName + "'; fallback to component.load()",
                            null, "sap.ushell.services.AppConfiguration");
                        try {
                            oComponent = sap.ui.component.load({ url : sUrl, name : sComponentName });
                            oLocalMetadataComponent = oComponent.getMetadata();

                        } catch (e) {
                            jQuery.sap.log.warning("Could not receive metadata of component", null, "sap.ushell.services.AppConfiguration");
                        }
                    }
                    ...
                    if (oLocalMetadataComponent) {
                    ...
                    } else if (sap.ushell.utils.isApplicationTypeNWBCRelated(oApplication.applicationType)) {
                        ...
                        if (iIndexOfWdaApplicationUrlString >= 0) {
                            ...
                            oMetadata[sKey].libraryName = oApplication.url.substring(
                                (iIndexOfWdaApplicationUrlString + sWdaApplicationUrlString.length), oApplication.url.indexOf("/", (iIndexOfWdaApplicationUrlString + sWdaApplicationUrlString.length))
                            );
                        }
                        if (iIndexOfWebGUIApplicationUrlString >= 0) {
                            ...
                            oMetadata[sKey].libraryName = oApplication.url.substring(
                                iETransactionStart + sETransactionString.length, iETransactionEnd
                            ) + " (TCODE)";
                        }
                    } else {
