    ...
    var ShapeMarker = Control.extend('sap.viz.ui5.controls.chartpopover.ShapeMarker', {
        ...
        renderer : {
            render : function(oRm, oControl) {
                ...
                var posX = markerSize / 2, posY = posX, width = markerSize, height = markerSize;
                ...
                oRm.writeClasses();
                oRm.write('>');
                oRm.write('<svg width=' + width + 'px height=' + height + 'px ');
                oRm.write('>');
                if(oControl._isShowWithLine()){
                    var lineInfo = oControl.getLineInfo(),
                        lineColor = sap.ui.core.theming.Parameters.get(lineInfo.lineColor);
                    if(!lineColor){
                        lineColor = lineInfo.lineColor ? lineInfo.lineColor : oControl.getColor(); 
                    }
                    oRm.write("<line x1 = '0' y1='" + posY + "' x2 = '" + width + "' y2 = '" + posY + "' ");
                    oRm.write(" stroke = '" + lineColor + "' stroke-width = '2' ");
                    if(lineInfo.lineType === 'dotted'){
                ...
                if(props.type){
                    ...
                    var stroke = oControl.getStroke();
                    //Do not draw marker's stroke
//                    if(stroke && stroke.visible){
//                        oRm.write(" stroke = '"+stroke.color + "' stroke-width= '1px'");
//                    }
