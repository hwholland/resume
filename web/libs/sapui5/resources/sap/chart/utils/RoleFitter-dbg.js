...
sap.ui.define([
	...
	var validateFeeds = function(chartType, feedItems) {
		...
		if (!oResults.valid) {
			...
			if (bindings && Object.keys(bindings).every(function(item) {
					return bindings[item].allowMND && (!bindings[item].missing || bindings[item].missing === 1);
				})) {
	...
	var _mRoleFeedMapping = [
		...
		{"types":"dual_combination|dual_stacked_bar|100_dual_stacked_bar|dual_stacked_column|100_dual_stacked_column|dual_bar|dual_column|dual_line|dual_stacked_combination|dual_horizontal_stacked_combination","toViz":{"axis1":"valueAxis","axis2|axis3|axis4":"valueAxis2"}},
		...
		{"types":"timeseries_column","toViz":{"category":"timeAxis", "axis1|axis2|axis3|axis4":"valueAxis"}},
	...
	function fit(sChartType, aDimensions, aMeasures, aInResults, sSeriesFeedingId) {
		...
		aFeedItems._unused = unUsedDimsOrMsrs(aFeeds, aDimensions, aMeasures);
