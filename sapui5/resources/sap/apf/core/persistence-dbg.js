...
sap.apf.core.Persistence = function(oInject) {
	...
	this.createPath = function(sName, fnCallback, oExternalObject) {
		var oRequest;
		var oSerializablePath = oInject.instances.coreApi.serialize();
		if (oExternalObject) {
		...
		function fnRequestCallback(oResponse, oEntityTypeMetadata, messageObject) {
			if (messageObject) {
				...
				oInject.instances.messageHandler.check(oResponse && oResponse.data && oResponse.statusCode === 201 && oResponse.statusText === "Created", "Persistence create Path - proper response");
				fnCallback({
	...
	this.openPath = function(sAnalysisPathId, fnCallback, nActiveStep) {
		...
		function fnRequestCallback(oResponse, oEntityTypeMetadata, oMessageObject) {
			...
			if (oMessageObject) {
				oMessageObjectForUI = oInject.instances.messageHandler.createMessageObject({
					code : '5210'
				});
				oMessageObjectForUI.setPrevious(oMessageObject);
				oInject.instances.messageHandler.putMessage(oMessageObjectForUI);
			}
	...
	function sendRequest(oRequest, fnLocalCallback, sAnalysisPathId) {
		...
		var sUrl = oInject.instances.coreApi.getPersistenceConfiguration().path.service + "/" + oInject.instances.coreApi.getPersistenceConfiguration().path.entitySet;
		oRequest.headers = {
			"x-csrf-token" : oInject.instances.coreApi.getXsrfToken(oInject.instances.coreApi.getPersistenceConfiguration().path.service)
		};
	...
	function createPathFromReceivedData(oReceivedData, nActiveStep) {
		...
		function restorePutMessage() {
			oInject.instances.messageHandler.putMessage = fnPutMessageOrig;
		}
		function redefinePutMessage() {
			fnPutMessageOrig = oInject.instances.messageHandler.putMessage;
			oInject.instances.messageHandler.putMessage = function(oMessageObject) {

				var messageConf = oInject.instances.messageHandler.getConfigurationByCode(oMessageObject.getCode());
		...
		oInject.instances.coreApi.resetPath(true);
		redefinePutMessage();
		try {
			oInject.instances.coreApi.deserialize(oReceivedData.SerializedAnalysisPath);
		} catch (oError) {
			oInject.instances.coreApi.restoreOriginalPath();
	...
	function getMetadata() {
		var oConfig = oInject.instances.coreApi.getPersistenceConfiguration();
		var oEntityTypeMetadata = oInject.instances.coreApi.getEntityTypeMetadata(oConfig.path.service, oConfig.path.entitySet);
	...
	function resolveLogicalSystemWithSapClient(deferred, sapClient) {
		...
		var logicalSystemConfiguration = oInject.instances.coreApi.getPersistenceConfiguration().logicalSystem;
		...
		sUrl = sUrl + oInject.instances.coreApi.getUriGenerator().buildUri(messageHandler, sEntityType, [ 'LogicalSystem' ], oFilter, undefined, undefined, undefined, undefined, undefined, 'Results');
		var oRequest = {
			requestUri : sUrl,
			method : "GET",
			headers : {
				"x-csrf-token" : oInject.instances.coreApi.getXsrfToken(sServiceRoot)
			}
		};
