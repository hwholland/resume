...
jQuery.sap.require('sap.viz.ui5.types.legend.Common_title');
/**
 *@class stepToolbar
 *@name stepToolbar
...
(function() {
    ...
    sap.ui.controller("sap.apf.ui.reuse.controller.stepToolbar", {
        ...
        insertViewSettingsIcon: function() {
            var self = this;
            var oViewSettingDialog, oTableRepresentation;
            this.oSelectedRepresentation = this.oCoreApi.getActiveStep().getSelectedRepresentation(); // get the selected representation as the view data
            //if the selected representation is table , create a view setting for it
            if (this.oSelectedRepresentation.toggleInstance || this.oSelectedRepresentation.type === sap.apf.ui.utils.CONSTANTS.representationTypes.TABLE_REPRESENTATION) {
                oTableRepresentation = this.oSelectedRepresentation.toggleInstance || this.oSelectedRepresentation; // either toggled instance of table or the table representation
                oViewSettingDialog = oTableRepresentation.oViewSettingDialog ? oTableRepresentation.oViewSettingDialog : oTableRepresentation.createViewSettingDialog(); //create the view setting dialog for table
            }
            this.sortButton = new sap.m.Button({
                icon: "sap-icon://drop-down-list",
                tooltip: this.oCoreApi.getTextNotHtmlEncoded("view-Settings-Button"),
                press: function() {
                    if (self.oSelectedRepresentation.toggleInstance || (self.oSelectedRepresentation.type === sap.apf.ui.utils.CONSTANTS.representationTypes.TABLE_REPRESENTATION && self.oSelectedRepresentation.topN === undefined)) {
        ...
        drawMultipleRepresentation: function() {
            ...
            //full-screen buttons
            function onPressOfRepresentatioType(oEvent) {
            ...
            for (var k = 0; k < reperesentationTypesLength; k++) {
                var button = new sap.m.Button({
                    ...
                    press: onPressOfRepresentatioType
                });
        ...
        drawRepresentation: function(oChart) {
            ...
            if (this.getView().chartToolbar.getFullScreen() === true) {
                ...
                if (this.oCoreApi.getActiveStep().getSelectedRepresentation().bIsAlternateView || this.oCoreApi.getActiveStep().getSelectedRepresentation().type === sap.apf.ui.utils.CONSTANTS.representationTypes.TABLE_REPRESENTATION) {
                    var aRequiredFilter = this.oCoreApi.getActiveStep().getSelectedRepresentation().getParameter().requiredFilters;
