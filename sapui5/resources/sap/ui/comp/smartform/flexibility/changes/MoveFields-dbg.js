...
sap.ui.define([
	 ...
	 * @param {object} mPropertyBag.view
	 * @param {object} mPropertyBag.modifier
	 * @param {object} mPropertyBag.appComponent
	 * @public
	 * @function
	...
	MoveFields.applyChange = function (oChange, oGroup, mPropertyBag) {
		...
		function getFieldControlOrThrowError(oMoveField, oModifier) {
			...
			return oModifier.bySelector(oMoveField.selector || oMoveField.id, mPropertyBag.appComponent, mPropertyBag.view);
		}
		...
		oChangeContent.moveFields.forEach(function (oMoveField) {
			var oField = getFieldControlOrThrowError(oMoveField, oModifier);

			if (!oField) {
				FlexUtils.log.warning("Element to move not found in source group");
				return;
			}
