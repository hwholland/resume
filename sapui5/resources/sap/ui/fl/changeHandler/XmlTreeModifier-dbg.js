...
sap.ui.define(["sap/ui/fl/changeHandler/BaseTreeModifier"], function (BaseTreeModifier) {
		...
		var XmlTreeModifier = {
			...
			_byId: function (sId, oView) {
				if (oView) {
					if (oView.ownerDocument && oView.ownerDocument.getElementById) {
						return jQuery(oView).find("#" + sId)[0];
						// Fails under IE 11.420 indicating "permission denied": return oView.ownerDocument.getElementById(sId);
					} else if (oView.getElementById) {
						var oNode = oView.getElementById(sId);
						if (oNode) {
							return oNode;
						}
					}
