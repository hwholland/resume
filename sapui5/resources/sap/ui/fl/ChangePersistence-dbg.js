...
sap.ui.define([
	...
	ChangePersistence.prototype.getUnmergedChangesForView = function(mPropertyBag) {
		return this.getChangesForView(mPropertyBag).then(this._getUnmergedChanges.bind(this));
	};
	...
	ChangePersistence.prototype.getChangesForComponent = function(mPropertyBag) {
		...
		return Cache.getChangesFillingCache(this._oConnector, this._sComponentName, mPropertyBag).then(function(oWrappedChangeFileContent) {
			...
			Settings._storeInstance(that._sComponentName, oWrappedChangeFileContent);
			...
			return ContextManager.getActiveContexts(aContextObjects).then(function (aActiveContexts) {
				return aChanges.filter(preconditionsFulfilled.bind(that, aActiveContexts)).map(createChange);
			});
		});
