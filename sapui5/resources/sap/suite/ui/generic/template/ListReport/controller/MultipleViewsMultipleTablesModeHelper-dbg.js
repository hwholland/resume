sap.ui.define(["jquery.sap.global", "sap/ui/base/Object"],
	function(jQuery, BaseObject) {
		...
		function getMethods(oQuickVariantSelectionX, oState, oController, oTemplateUtils, fnSetInitialKey, mItemData) {
			...
			function fnSetTablesToDirtyByEntitySet(sEntitySet, bDirty) {
				var oItem;
				var bState = bDirty || false;
				if (!sEntitySet) {
					return;
				}
				for (var i in mItemData) {
						oItem = mItemData[i];
					if (oItem.implementingControl.getProperty("entitySet") === sEntitySet) {
						mDirtyMap[oItem.id] = bState;
					}
				}
			}
			...
			return {
				...
				init: fnInit,
				setTablesToDirtyByEntitySet: fnSetTablesToDirtyByEntitySet
			};
		}
