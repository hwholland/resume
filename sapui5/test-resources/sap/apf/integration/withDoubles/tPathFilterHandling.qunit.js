...
(function() {
	...
	function commonSetup(oContext) {
			...
			sap.apf.testhelper.createComponent(oContext, 
					{ stubAjaxForResourcePaths : true, doubleUiInstance : true,  path : "pathOfNoInterest",  inject : inject});
	...
	QUnit.test("Path with first step with selection and 2 further steps with a selection, delete step in middle, which has a selection and test filter update", function(assert) {
		function isContainedInArray(aContained, aData) {
			...
			var bFound;
			for(var i = 0; i < aContained.length; i++) {
		...
		var bContains = isContainedInArray(aExpectedData, aDataOfLastStepRepresentationAfterSelectionChange);
	...
	QUnit.test("Update fails on existing path", function(assert) {
		...
		this.oApi.setCallbackForMessageHandling(assertCorrectErrorMessage);
