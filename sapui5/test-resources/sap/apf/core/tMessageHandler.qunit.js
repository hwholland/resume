...
(function() {
	'use strict';
QUnit.module('MH Message Handler', {
...
QUnit.test("WHEN loadConfig used without reset THEN duplicate error message", function(assert) {
	function assertMessageForDuplicateCode(oMessageObject) {
		...
		assert.equal(sCode, sap.apf.core.constants.message.code.errorStartUp);
...
QUnit.test('GIVEN set callback WHEN putMessage THEN callback called', function(assert) {
	...
	// TODO better test needed for checking, if message handler works correct without callback function
	sMessageCode = "";
...
QUnit.test('Message code and text for the 900X messages', function(assert) {
	...
	this.assertMessageInLog(assert, nLogEntriesBeforeMessage, "9004 - raw text param1", "Correct number and raw text");
});
...
QUnit.test('Creation of message with many previous messages - only 10 are displayed and handled!', function(assert) {
	...
	assert.ok(sMessage.search(sap.apf.core.constants.message.code.errorStartUp) == -1, "not more than 10 messages");
...
QUnit.test("Additional message callback (second callback)", function(assert) {
	var fnFirstCallback = function(oMessageObject, fnSecondCallback) {
		assert.equal(oMessageObject.getCode(), 5002, "correct message code");
		assert.ok(typeof (fnSecondCallback) === "function", "Second callback in first callback included");
		fnSecondCallback(oMessageObject);
	};
	var fnSecondCallback = function(oMessageObject) {
		assert.equal(oMessageObject.getCode(), 5002, "correct message code");
	};
	this.oMessageHandler.setMessageCallback(fnFirstCallback);
	this.oMessageHandler.setApplicationMessageCallback(fnSecondCallback);
	this.oResourcePathHandler = new sap.apf.testhelper.doubles.ResourcePathHandler({
		instances : {
			messageHandler : this.oMessageHandler
		}
...
QUnit.test("Register second callback before the first callback", function(assert) {
	var fnFirstCallback = function(oMessageObject, fnSecondCallback) {
		assert.equal(oMessageObject.getCode(), 5002, "correct message code");
		assert.ok(typeof (fnSecondCallback) === "function", "Second callback in first callback included");
		fnSecondCallback(oMessageObject);
...
QUnit.test('Register non-function application message callback creates log message', function(assert) {

	var nLogEntriesBeforeMessage = this.getLogEntries().length;
	this.oMessageHandler.loadConfig(sap.apf.core.messageDefinition);
	this.oMessageHandler.setApplicationMessageCallback({
		something : 'something'
	});
	this.assertMessageInLog(assert, nLogEntriesBeforeMessage, "5031 - Argument for method 'setApplicationMessageCallback' is not a function.", "Correct warning message been logged.");
});
...
QUnit.test("Create message if an exception in TextResourceHandler occurs", function(assert) {
	...
	assert.equal(sMessageText, "Data not available for step Hugo", "Message text as expected for an exception in text ressource handler");
});
...
QUnit.test("WHEN error is put, and text ressource handler raises exception", function(assert){
	...
	messageHandler.loadConfig(sap.apf.core.messageDefinition, true);
	var fnCallback = function(messageObject) {
		assert.equal(messageObject.getMessage(), "You must log out of the application due to a critical error.", "THEN text from description is taken as default");
	};
	messageHandler.setMessageCallback(fnCallback);
	var messageObject = messageHandler.createMessageObject({ code : 5300});
	assert.throws(function() {
		messageHandler.putMessage(messageObject);
