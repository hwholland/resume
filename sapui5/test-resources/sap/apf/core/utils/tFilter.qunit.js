...
(function() {
	...
	function commonSetup(o) {
		...
		o.oMsgHandler = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging();
		o.Filter = sap.apf.core.utils.Filter;
		defineFilterOperators();
		o.getCountryCityExpr = function() {
			var oFilter1 = new this.Filter(this.oMsgHandler, "country", this.EQ, "BRA");
			var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
			var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.EQ, "RUS");
			oFilter1.addAnd(new this.Filter(this.oMsgHandler, "city", this.EQ, "RIO"));
			oFilter2.addAnd(new this.Filter(this.oMsgHandler, "city", this.EQ, "BUE"));
			oFilter3.addAnd(new this.Filter(this.oMsgHandler, "city", this.EQ, "MOS"));
			var oFilter0 = new this.Filter(this.oMsgHandler, oFilter1);
			oFilter0.addOr(oFilter2);
			oFilter0.addOr(oFilter3);
			return oFilter0;
		};
		o.getCountryOrExpr = function() {
			var oFilter = new this.Filter(this.oMsgHandler, "country", this.EQ, "BRA");
			var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
			var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.EQ, "RUS");
	...
	QUnit.test("filter creation", function(assert) {
		var testObj = this;
		var oEmptyFilter = new this.Filter(this.oMsgHandler);
		...
		oFilter = new this.Filter(this.oMsgHandler, "country", this.NE, "BRA");
		assert.ok(oFilter, "Not equals");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.GT, "BRA");
		assert.ok(oFilter, "Greater than");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.LT, "BRA");
		assert.ok(oFilter, "Lower than");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		assert.ok(oFilter, "Lower equals");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.GE, "BRA");
		assert.ok(oFilter, "Greater equals");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.BT, "BRA", "RUS");
		assert.ok(oFilter, "Between");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.StartsWith, "BR");
		assert.ok(oFilter, "StartsWith");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.EndsWith, "RA");
		assert.ok(oFilter, "EndsWith");
		oFilter = new this.Filter(this.oMsgHandler, "country", this.Contains, "RA");
	...
	QUnit.test("filter creation with all supported operators", function(assert) {
		var oFilter;
		var i;
		for(i = 0; i < sap.apf.core.constants.aSelectOpt.length; i++) {
			oFilter = new this.Filter(this.oMsgHandler, "country", sap.apf.core.constants.aSelectOpt[i], "BRA");
			assert.ok(oFilter);
		}
	});
	...
	QUnit.test("addAnd - adding to empty filter", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler);
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
		var oFilterCompare = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
	...
	QUnit.test('Method "toUrlParam()" - Between', function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.BT, "BRA", "RUS");
		var sParam = oFilter.toUrlParam();
	...
	QUnit.test('Method "toUrlParam()" - simple disjunction', function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.EQ, "BRA");
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
		var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.LE, "RUS");
		var oFilter4 = new this.Filter(this.oMsgHandler, "country", this.StartsWith, "RU");
		oFilter.addOr(oFilter2);
		oFilter.addOr(oFilter3);
	...
	QUnit.test('Method "toUrlParam()" - simple disjunction with additional empty filter added after first term', function(assert) {
		...
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
		var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.LE, "RUS");
		oFilter.addOr(new this.Filter(this.oMsgHandler));
		oFilter.addOr(oFilter2);
		oFilter.addOr(oFilter3);
		var sParam = oFilter.toUrlParam();
	...
	QUnit.test('Method "toUrlParam()" - one expression with value type number literal', function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "property", this.EQ, 4711);
		var sParam = oFilter.toUrlParam();
	...
	QUnit.test('Method "toUrlParam()" - simple disjunction with value type number literal includede in middle term', function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "property", this.EQ, "BRA");
		var oFilter2 = new this.Filter(this.oMsgHandler, "property", this.EQ, 4711);
		var oFilter3 = new this.Filter(this.oMsgHandler, "property", this.LE, "RUS");
		oFilter.addOr(new this.Filter(this.oMsgHandler));
		oFilter.addOr(oFilter2);
		oFilter.addOr(oFilter3);
		var sParam = oFilter.toUrlParam();
	...
	QUnit.test("isConsistentWithFilter() on LT", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LT, "BRA");
	...
	QUnit.test("isConsistentWithFilter() on type date", function(assert) {
		...
		var oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.GT, new Date('2011-11-11'));
		...
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.GE, new Date('2011-11-11'));
		...
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.EQ, new Date('2011-11-11'));
		...
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.NE, new Date('2011-11-11'));
		isConsistent = oFilter.isConsistentWithFilter('dateProperty', new Date('2011-11-11'));
		...
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.LE, new Date('2011-11-11'));
		isConsistent = oFilter.isConsistentWithFilter('dateProperty', new Date('2012-11-11'));
		assert.equal(isConsistent, false, "Later Date not matched");
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.LT, new Date('2011-11-11'));
		isConsistent = oFilter.isConsistentWithFilter('dateProperty', new Date('2011-11-11'));
		assert.equal(isConsistent, false, "Same Date not matched");
		isConsistent = oFilter.isConsistentWithFilter('dateProperty', new Date('2012-11-11'));
		assert.equal(isConsistent, false, "Later Date not matched");
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.BT, new Date('2011-11-10'), new Date('2011-11-13'));
		isConsistent = oFilter.isConsistentWithFilter('dateProperty', new Date('2011-11-12'));
		assert.equal(isConsistent, true, "Date matched");
	});
	QUnit.test("boolean filter property", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "booleProperty", this.EQ, true);
		...
		oFilter = new this.Filter(this.oMsgHandler, "booleProperty", this.NE, true);
	...
	QUnit.test("EndsWith", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.EndsWith, "RA");
		var isConsistent = oFilter.isConsistentWithFilter('country', "BRA");
		assert.equal(isConsistent, true, "BRA  matched");
	...
	QUnit.test("Contains", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.Contains, "RA");
		var isConsistent = oFilter.isConsistentWithFilter('country', "BRA");
		assert.equal(isConsistent, true, "BRA  matched");
		isConsistent = oFilter.isConsistentWithFilter('country', "RA");
		assert.equal(isConsistent, true, "Original value RA matched");
	...
	QUnit.test("GT", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.GT, "BRA");
		var isConsistent = oFilter.isConsistentWithFilter('country', "BRA");
		assert.equal(isConsistent, false, "BRA not matched");
	...
	QUnit.test("NE", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.NE, "BRA");
		var isConsistent = oFilter.isConsistentWithFilter('country', "BRA");
		assert.equal(isConsistent, false, "BRA not matched");
		isConsistent = oFilter.isConsistentWithFilter('country', "BRZ");
		assert.equal(isConsistent, true, "BR1 matched");
	...
	QUnit.test("Or(eq,eq), same properties, match both values", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "A", this.EQ, "1");
		var oFilter2 = new this.Filter(this.oMsgHandler, "A", this.EQ, "2");
	...
	QUnit.test("different property names", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "A", this.LE, "1");
		var oFilter2 = new this.Filter(this.oMsgHandler, "XXX", this.EQ, "777");
		var oFilter3 = new this.Filter(this.oMsgHandler, "B", this.GE, "1000");
	...
	QUnit.test("OR(A=_,_), left matches property and value match in left branch", function(assert) {
		var leftTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '1');
		var rightTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'X', this.EQ, '2');
		var oFilter = new this.Filter(this.oMsgHandler, leftTerm).addOr(rightTerm);
	...
	QUnit.test("OR(A=_,A=_), match in right branch only", function(assert) {
		var leftTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '1');
		var rightTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '2');
		var oFilter = new this.Filter(this.oMsgHandler, leftTerm).addOr(rightTerm);
		this.leftTermSpy = sinon.stub(leftTerm, 'isConsistentWithFilter', function() {
			return false;
		});
	...
	QUnit.test("OR(A=_,B=_), property does not match left branch and does not match any value", function(assert) {
		var leftTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '1');
		var rightTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'B', this.EQ, '2');
		var oFilter = new this.Filter(this.oMsgHandler, leftTerm).addOr(rightTerm);
		this.leftTermSpy = sinon.stub(leftTerm, 'isConsistentWithFilter', function() {
			return false;
		});
	...
	QUnit.test("OR(A=1,A=2,C=x), test with A=4", function(assert) {
		var term1 = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '1');
		var term2 = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '2');
		var term3 = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'C', this.EQ, 'x');
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler, term1).addOr(term2).addOr(term3);
	...
	QUnit.test("Filter(FilterTerm), property is not constrained", function(assert) {
		var filterTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '1');
		var oFilter = new this.Filter(this.oMsgHandler, filterTerm);
	...
	QUnit.test("AND(_,_), match both branches", function(assert) {
		var leftTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '1');
		var rightTerm = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, 'A', this.EQ, '2');
		var oFilter = new this.Filter(this.oMsgHandler, leftTerm).addAnd(rightTerm);
		this.leftTermSpy = sinon.stub(leftTerm, 'isConsistentWithFilter', function() {
			return true;
		});
		this.rightTermSpy = sinon.stub(rightTerm, 'isConsistentWithFilter', function() {
	...
	QUnit.test("equal of two filters", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter2 = new this.Filter(this.oMsgHandler, "SAPClient", this.EQ, "777");
		var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.NE, "BRA");
		var oFilter4 = new this.Filter(this.oMsgHandler, "country", this.LE, "BRX");
	...
	QUnit.test("Equality on boolean values", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "booleanProperty", this.EQ, true);
		var oFilter1 = new this.Filter(this.oMsgHandler, "booleanProperty", this.EQ, true);
		var bEqual = oFilter.isEqual(oFilter1);
		assert.equal(bEqual, true, "filter are equal");
		oFilter = new this.Filter(this.oMsgHandler, "booleanProperty", this.EQ, true);
		oFilter1 = new this.Filter(this.oMsgHandler, "booleanProperty", this.EQ, false);
		bEqual = oFilter.isEqual(oFilter1);
		assert.equal(bEqual, false, "filter are not equal");
	});
	QUnit.test("Equality on date values", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.EQ, new Date('2011-11-11'));
		var oFilter1 = new this.Filter(this.oMsgHandler, "dateProperty", this.EQ, new Date('2011-11-11'));
		var bEqual = oFilter.isEqual(oFilter1);
		assert.equal(bEqual, true, "filter are equal");
		oFilter = new this.Filter(this.oMsgHandler, "dateProperty", this.EQ, new Date('2011-11-11'));
		oFilter1 = new this.Filter(this.oMsgHandler, "dateProperty", this.EQ, new Date('2011-11-12'));
		bEqual = oFilter.isEqual(oFilter1);
		assert.equal(bEqual, false, "filter are not equal");
	});
	QUnit.test("Equality of copied filters and addition of empty filter to one filter", function(assert) {
		var oFilter1 = new this.Filter(this.oMsgHandler, "Customer", this.EQ, "1001");
		oFilter1.addAnd("Customer", this.EQ, "1002");
		var oFilter2 = oFilter1.copy();
		oFilter2.addAnd(new this.Filter(this.oMsgHandler));
	...
	QUnit.test("Equality of copied filter terms with BT", function(assert) {
		var oFilterTerm1 = new sap.apf.core.utils.FilterTerm(this.oMsgHandler, "Customer", this.BT, "1001", "1003");
	...
	QUnit.test("commutative equal of two filters 1 - the order of terms shouln't matter", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "country", this.LE, "GER");
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.LE, "GER");
		var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
	...
	QUnit.test("commutative equal of two filters 2 - the order of filters shouln't matter", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.LE, "RIO");
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.LE, "GER");
		var oFilter3 = new this.Filter(this.oMsgHandler, "city", this.LE, "WDF");
		oFilter.addAnd(oFilter1);
		var oFilter4 = new this.Filter(this.oMsgHandler, oFilter);
		oFilter2.addAnd(oFilter3);
		oFilter4.addOr(oFilter2); // left side
		var oFilter_ = new this.Filter(this.oMsgHandler, "city", this.LE, "WDF");
		var oFilter1_ = new this.Filter(this.oMsgHandler, "country", this.LE, "GER");
		var oFilter2_ = new this.Filter(this.oMsgHandler, "city", this.LE, "RIO");
		var oFilter3_ = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		oFilter1_.addAnd(oFilter_);
		var oFilter4_ = new this.Filter(this.oMsgHandler, oFilter1_);
	...
	QUnit.test("and/or on different levels", function(assert) {
		...
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.EQ, "ARG");
		var oFilter3 = new this.Filter(this.oMsgHandler, "country", this.EQ, "RUS");
		var oFilterOr = new this.Filter(this.oMsgHandler, oFilter1).addOr(oFilter2);
		var oFilterAnd = new this.Filter(this.oMsgHandler, oFilter3).addAnd(oFilterOr);
		var oFilterAndCompare = new this.Filter(this.oMsgHandler, oFilter1).addAnd(oFilter2);
		var oFilterOrCompare = new this.Filter(this.oMsgHandler, oFilter3).addOr(oFilterAndCompare);
		assert.equal(oFilterAnd.isEqual(oFilterOrCompare), false, "and and or on different levels handled correct way");
		var oFilterOr2 = new this.Filter(this.oMsgHandler, oFilter2).addOr(oFilter1);
		var oFilterAnd2 = new this.Filter(this.oMsgHandler, oFilter3).addAnd(oFilterOr2);
		assert.equal(oFilterAnd.isEqual(oFilterAnd2), true, "commutative or handled correct way");
		var oFilterAndCompare2 = new this.Filter(this.oMsgHandler, oFilter2).addAnd(oFilter1);
		var oFilterOrCompare2 = new this.Filter(this.oMsgHandler, oFilter3).addOr(oFilterAndCompare2);
	...
	QUnit.module('Filter & EmptyFilter related to simplifyFilter', {
		beforeEach : function(assert) {
			jQuery.extend(this,	sap.apf.core.constants.FilterOperators);
			this.oMsgHandler = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging();
			this.Filter = sap.apf.core.utils.Filter;
		},
		afterEach : function(assert) {
		}
	});
	QUnit.test( 'EmptyFilter', function(assert) {
		...
		assert.strictEqual( filter instanceof sap.apf.core.utils.Filter, true, 'type');
	...
	QUnit.test( 'Filter pointing to EmptyFilter', function(assert) {
		var emptyFilter = new sap.apf.core.utils.Filter(this.oMsgHandler);
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler);
	...
	QUnit.test( 'AND(EmptyFilter, eq)', function(assert) {
		...
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler);
		filter.addAnd(emptyFilter);
		var eq = new sap.apf.core.utils.Filter(this.oMsgHandler, "A", this.EQ, 1);
		filter.addAnd(eq);
		assert.strictEqual( filter instanceof sap.apf.core.utils.Filter, true, 'type');
	...
	QUnit.test( 'Filter eq', function(assert) {
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler, "A", this.EQ, 1);
	...
	QUnit.test('AND(eq, eq)', function(assert) {
		var filter = new this.Filter(this.oMsgHandler, "A", this.EQ, "1").addAnd("A", this.EQ, "2");
	...
	QUnit.test( 'AND(EmptyFilter, EmptyFilter)', function(assert) {
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler);
		var emptyFilter1 = new sap.apf.core.utils.Filter(this.oMsgHandler);
		filter.addAnd(emptyFilter1);
		var emptyFilter2 = new sap.apf.core.utils.Filter(this.oMsgHandler);
	...
	QUnit.test( 'AND(EmptyFilter, EmptyFilter, EmptyFilter)', function(assert) {
		...
		var emptyFilter2 = new sap.apf.core.utils.Filter(this.oMsgHandler);
		filter.addAnd(emptyFilter2);
		var emptyFilter3 = new sap.apf.core.utils.Filter(this.oMsgHandler);
	...
	QUnit.test( 'AND(EmptyFilter, AND(EmptyFilter, EmptyFilter))', function(assert) {
		...
		var emptyFilter1 = new sap.apf.core.utils.Filter(this.oMsgHandler);
		filter.addAnd(emptyFilter1);
		var andNode = new sap.apf.core.utils.Filter(this.oMsgHandler);
		var emptyFilter2 = new sap.apf.core.utils.Filter(this.oMsgHandler);
	...
	QUnit.test( 'Filter OR(eq, eq)', function(assert) {
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler, 'A', this.EQ, "de");
		var sub = new sap.apf.core.utils.Filter(this.oMsgHandler, 'A', this.EQ, "fr");
		filter.addOr(sub);
		assert.strictEqual( filter instanceof sap.apf.core.utils.Filter, true, 'type');
		assert.strictEqual( filter.getFilterTerms().length, 2, 'getFilterTerms().length' );
		assert.strictEqual( filter.isEmpty(), false, 'isEmpty');
	...
	QUnit.test( 'Filter AND(OR(eq, eq)) which is a wrapper around OR, the subtree is in oLeftExpr', function(assert) {
		var filter = new sap.apf.core.utils.Filter(this.oMsgHandler);
		var orNode = new sap.apf.core.utils.Filter(this.oMsgHandler, 'A', this.EQ, "de");
		var sub = new sap.apf.core.utils.Filter(this.oMsgHandler, 'A', this.EQ, "fr");
		orNode.addOr(sub);
	...
	QUnit.test("Conjunction of two terms - remove term and compare with second filter", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.LE, "RIO");
		var oFilter2 = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		oFilter.addAnd(oFilter1);
	...
	QUnit.test("One term with BT", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.BT, "ARG", "BRA");
	...
	QUnit.test("One term with BT", function(assert) {
		var oFilter1 = new this.Filter(this.oMsgHandler, "country", this.BT, "ARG", "BRA");
		var oFilter2 = new this.Filter(this.oMsgHandler, "city", this.BT, "HAM", "MUNICH");
		oFilter1.addAnd(oFilter2);
		var oFilterReduced = oFilter1.removeTermsByProperty("city");
		var oFilterCompare = new this.Filter(this.oMsgHandler, "country", this.BT, "ARG", "BRA");
	...
	QUnit.test("Removal of several terms from a filter like constructed in the path update", function(assert) {
		var oStartFilter = new this.Filter(this.oMsgHandler, "GlobalProp1", this.EQ, 'value1');
		oStartFilter.addAnd(new this.Filter(this.oMsgHandler, "GlobalProp2", this.EQ, 'value2'));
		var oCummulativeFilter = new this.Filter(this.oMsgHandler, oStartFilter);
		var oFilterFromStep = new this.Filter(this.oMsgHandler);
		var i;
		for(i = 0; i < 3; i++) {
			oFilterFromStep.addOr(new this.Filter(this.oMsgHandler, "StepProp1", this.EQ, 'value' + i));
		}
	...
	QUnit.test("Nested filter - two level", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, 'PropertyToExtract', this.EQ, '10');
		oFilter.addOr('p_param2', this.EQ, '4711');
		oFilter.addOr('PropertyOfNoInterestTwo', this.LT, '200');
		var oNextLevelFilter = new this.Filter(this.oMsgHandler, 'PropertyToExtract', this.EQ, '20');
		oNextLevelFilter.addOr('PropertyOfNoInterestOne', this.GT, '120');
		oNextLevelFilter.addOr('p_param2', this.EQ, '4712');
		var oCombinedFilter = new this.Filter(this.oMsgHandler, oFilter);
	...
	QUnit.test("Nested filter - compound filter (three level)", function(assert) {
		var oCompound = new this.Filter(this.oMsgHandler, 'prop1', this.EQ, 'val1');
		oCompound.addAnd('prop2', this.EQ, 'val2');
		oCompound.addAnd('prop3', this.EQ, 'val3');
		var oFilter = new this.Filter(this.oMsgHandler, oCompound);
		oFilter.addOr(oCompound);
		var oNextLevelFilter = new this.Filter(this.oMsgHandler, oCompound);
	...
	QUnit.test("Filter reduction to not existing properties returns an initial filter object", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.EQ, "Berlin");
		var oFilter2 = new this.Filter(this.oMsgHandler, "street", this.EQ, "Kurfürstendamm");
		var oFilterExpected = new this.Filter(this.oMsgHandler);
		oFilter.addAnd(oFilter1).addAnd(oFilter2);
		var oFilterReduction = oFilter.reduceToProperty("hugo");
	...
	QUnit.test("Filter reduction to existing properties returns a new instance of the filter", function(assert) {
		...
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.EQ, "Berlin");
		oFilter.addAnd(oFilter1);
		var oFilterReduction = oFilter.reduceToProperty("country");
		assert.notEqual(oFilterReduction, oFilter, "New instance returned");
	});
	QUnit.test("Call Type I: Filter projected to one property parameter", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.EQ, "Berlin");
		var oFilter2 = new this.Filter(this.oMsgHandler, "street", this.EQ, "Kurfürstendamm");
	...
	QUnit.test("Call Type II: Filter reduced to two several parameters", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.EQ, "Berlin");
		var oFilter2 = new this.Filter(this.oMsgHandler, "street", this.EQ, "Kurfürstendamm");
		oFilter.addAnd(oFilter1);
		var oFilterExpected = oFilter.copy();
		oFilter.addAnd(oFilter2);
		var oFilterReduction = oFilter.reduceToProperty("country", "city");
		var bEqual = oFilterReduction.isEqual(oFilterExpected);
		assert.equal(bEqual, true, "filter equal");
	});
	QUnit.test("Call Type III: Filter reduced to an array of properties", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, "country", this.LE, "BRA");
		var oFilter1 = new this.Filter(this.oMsgHandler, "city", this.EQ, "Berlin");
		var oFilter2 = new this.Filter(this.oMsgHandler, "street", this.EQ, "Kurfürstendamm");
		oFilter.addAnd(oFilter1);
	...
	QUnit.test("copy of filters", function(assert) {
		...
		oFilter1 = new this.Filter(this.oMsgHandler);
	...
	QUnit.test("create selection expression from array 1", function(assert) {
		...
		var oFilter = this.Filter.createFromArray(this.oMsgHandler, [ 'country', 'city' ], data, [ 0, 1, 2 ]);
	...
	QUnit.test("create selection expression from array 2", function(assert) {
		...
		var oFilter = this.Filter.createFromArray(this.oMsgHandler, [ 'country' ], data, [ 1, 3, 4 ]);
	...
	QUnit.test("create selection expression from  array 3", function(assert) {
		...
		var oFilter = this.Filter.createFromArray(this.oMsgHandler, [ 'country' ], data, [ 0, 1, 2 ]);
		var oFilterCompare = this.getCountryOrExpr();
		var bEqual = oFilterCompare.isEqual(oFilter);
	...
	QUnit.test("create selection expression from array superflous line in indices", function(assert) {
		...
		// city was changed
		var oFilter = this.Filter.createFromArray(this.oMsgHandler, [ 'country', 'city' ], data, [ 0, 1, 2, 3, 5 ]);
		var oFilterCompare = this.getCountryCityExpr();
		var bEqual = oFilterCompare.isEqual(oFilter);
		assert.equal(bEqual, false, "constructed selections  with different data not equal");
	});
	QUnit.test("create selection expression from array - no indices", function(assert) {
		...
		// city was changed
		var oFilter = this.Filter.createFromArray(this.oMsgHandler, [ 'country', 'city' ], data, []);
		var oFilterCompare = new this.Filter(this.oMsgHandler);
		var bEqual = oFilterCompare.isEqual(oFilter);
	...
	QUnit.test("Middle term twice from conjunction", function(assert) {
		var oFilter = new this.Filter(this.oMsgHandler, 'PropertyOfNoInterestTwo', this.EQ, '10');
		oFilter.addOr('PropertyToExtract', this.GT, '100');
		oFilter.addOr('PropertyOfNoInterestTwo', this.LT, '200');
		var oNextLevelFilter = new this.Filter(this.oMsgHandler, 'PropertyOfNoInterestTwo', this.EQ, '20');
	...
	QUnit.test("Basic test", function(assert) {
		var oFilter = sap.apf.core.utils.Filter.createEmptyFilter(this.oMsgHandler, [ 'PropertyOfNoInterest' ]);
		var oFilterCompare = new this.Filter(this.oMsgHandler, 'PropertyOfNoInterest', this.EQ, '');
		oFilterCompare.addAnd(new this.Filter(this.oMsgHandler, 'PropertyOfNoInterest', this.NE, ''));
	...
	QUnit.test("Overwrite with initial filter", function(assert) {
		var oFilter1 = new this.Filter(this.oMsgHandler, "PropertyOfNoInterestOne", this.EQ, "value1");
		var oFilter2 = new this.Filter(this.oMsgHandler);
		var oOverwrittenFilter = oFilter1.overwriteWith(oFilter2);
		var oFilterCompare = new this.Filter(this.oMsgHandler, "PropertyOfNoInterestOne", this.EQ, "value1");
	...
	QUnit.test("Overwrite with disjunctive filter", function(assert) {
		var oFilter1 = new this.Filter(this.oMsgHandler, "PropertyOfNoInterestOne", this.EQ, "value1");
		oFilter1.addAnd(new this.Filter(this.oMsgHandler, "PropertyOfNoInterestTwo", this.EQ, "value2"));
		var oFilter2 = new this.Filter(this.oMsgHandler, "PropertyOfNoInterestThree", this.EQ, "value3");
		var oFilterCompare = new this.Filter(this.oMsgHandler, oFilter1.copy()).addAnd(oFilter2);
		var oOverwrittenFilter = oFilter1.overwriteWith(oFilter2);
	...
	QUnit.test("Overwrite filter with same properties", function(assert) {
		var oFilter1 = new this.Filter(this.oMsgHandler, "PropertyOfNoInterestOne", this.EQ, "value1");
		oFilter1.addAnd(new this.Filter(this.oMsgHandler, "PropertyOfNoInterestTwo", this.EQ, "value2"));
		var oFilter2 = new this.Filter(this.oMsgHandler, "PropertyOfNoInterestOne", this.EQ, "value3");
		var oFilterCompare = new this.Filter(this.oMsgHandler, oFilter2.copy()).addAnd(new this.Filter(this.oMsgHandler, "PropertyOfNoInterestTwo", this.EQ, "value2"));
		var oOverwrittenFilter = oFilter1.overwriteWith(oFilter2);
		assert.ok(oOverwrittenFilter.isEqual(oFilterCompare), "Filter merged correctly");
	});
	...
	QUnit.test("Complex filter", function(assert) {
		var CostingAreaFilter1 = new this.Filter(this.oMsgHandler, "CoArea", sap.ui.model.FilterOperator.EQ, "1000");
		var CostCenterFilter1 = new this.Filter(this.oMsgHandler, "CostCenter", sap.ui.model.FilterOperator.EQ, "11000");
		var CostCenterCompoundFilter1 = CostCenterFilter1.addAnd(CostingAreaFilter1);
		var CostingAreaFilter2 = new this.Filter(this.oMsgHandler, "CoArea", sap.ui.model.FilterOperator.EQ, "2000");
		var CostCenterFilter2 = new this.Filter(this.oMsgHandler, "CostCenter", sap.ui.model.FilterOperator.EQ, "22000");
		var CostCenterCompoundFilter2 = CostingAreaFilter2.addAnd(CostCenterFilter2);
		var AllCostCenterCompoundFilter = new this.Filter(this.oMsgHandler, CostCenterCompoundFilter1).addOr(CostCenterCompoundFilter2);
		var CompanyCodeFilter = new this.Filter(this.oMsgHandler, "CompanyCode", sap.ui.model.FilterOperator.EQ, "1000");
		var YearFilter = new this.Filter(this.oMsgHandler, "Year", sap.ui.model.FilterOperator.BT, 2014, 2015);
		var topFilter = new this.Filter(this.oMsgHandler, AllCostCenterCompoundFilter).addAnd(CompanyCodeFilter);
		...
		assert.deepEqual(result, expected, "THEN json format as expected");
	});
	QUnit.test("Empty filter", function(assert) {
		var topFilter = new sap.apf.core.utils.Filter(this.oMsgHandler);
		var result = topFilter.mapToSapUI5FilterExpression();

		assert.deepEqual(result, {}, "THEN json format as expected");
	});
	QUnit.test("Simple filter term", function(assert) {
		var topFilter = new sap.apf.core.utils.Filter(this.oMsgHandler, "A", "EQ", "1");
		var result = topFilter.mapToSapUI5FilterExpression();
		var expected = {
			sPath: "A",
			sOperator: "EQ",
	...
	QUnit.test('AND	on top level with two terms', function(assert) {
		this.assertTransformToInternalAndBackResultInOrigin(assert, {
			bAnd : true,
			aFilters : [ this.equalityTermA, this.equalityTermB ]
		});
	});
		...
		this.assertTransformToInternalAndBackResultInOrigin(assert, {
			...
			aFilters : [ this.betweenTerm, {
				...
				aFilters : [ {
					...
					bAnd : true,
					aFilters : [ this.betweenTerm, this.equalityTermB ]
				}, this.betweenTerm ]
			} ]
		});
	});
	...
	QUnit.test('Filter contains conjunction with one disjunction over single terms', function(assert) {
		var orFilter = new this.Filter(this.oMsgHandler);
		orFilter.addOr('propertyOne', 'EQ', 'value1');
		orFilter.addOr('propertyOne', 'EQ', 'value2');

		var andFilter = new this.Filter(this.oMsgHandler);
	...
	QUnit.test('Filter contains disjunction of single terms', function(assert) {
		var filter = new this.Filter(this.oMsgHandler);
	...
	QUnit.test('Filter contains conjunction on second level', function(assert) {
		var filterAnd = new this.Filter(this.oMsgHandler); 
	 ...
	 */
	QUnit.module('sap.apf.core.utils.Filter.traverse() - contract with visitor', {
		beforeEach: function (/* assert */) {
			var context = this;
			jQuery.extend(this, sap.apf.core.constants.FilterOperators);
			...
			this.createFilter = function (name, op, value) { 									// optional params
				return new sap.apf.core.utils.Filter(context.messageHandler, name, op, value);
			};
			/**
			 * @constructor
			 */
	...
	QUnit.test('traverse Filter(empty filter) - shall call visitor.processEmptyFilter(). Traverse shall not pass indirection to visitor', function (assert) {
		...
		var filter = new sap.apf.core.utils.Filter( this.messageHandler, emptyFilter);
	...
	QUnit.module('Filter.mapToSelectOptions()', {
		beforeEach : function (assert){
			this.msgH = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging();
	...
	QUnit.test('When filter is empty', function (assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH);
	...
	QUnit.test('Simple equation', function(assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH, 'A', 'eq', '1');
		var expect = {
				Parameters: [],
				SelectOptions: [{
					PropertyName : 'A',
	...
	QUnit.test('Simple range', function(assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH, 'A', 'bt', '1', '2');
		var expect = {
				Parameters: [],
				SelectOptions: [{
					PropertyName : 'A',
	...
	QUnit.test('Range with high = 0', function(assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH, 'A', 'bt', '1', 0);
		var expect = {
				Parameters: [],
				SelectOptions: [{
					PropertyName : 'A',
	...
	QUnit.test('Range with high = undefined', function(assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH, 'A', 'bt', '1', undefined);
		var expect = {
				Parameters: [],
				SelectOptions: [{
					PropertyName : 'A',
	...
	QUnit.test('Conjunction over disjunctions', function(assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH);
		var filter1 = new sap.apf.core.utils.Filter(this.msgH);
		var filter2 = new sap.apf.core.utils.Filter(this.msgH);
	...
	QUnit.test('Filter contains only one parameter', function(assert){
		var filter = new sap.apf.core.utils.Filter(this.msgH, 'pA', 'eq', '1');
		var expect = {
				Parameters: [{
					PropertyName: 'pA',
					PropertyValue: '1'
