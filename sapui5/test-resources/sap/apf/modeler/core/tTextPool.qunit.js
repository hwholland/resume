...
(function() {
	'use strict';
	function commonSetupTextPool(context, applicationId, expectedUuid, odataDoChangeOperationsInBatch) {
	...
	QUnit.test("set empty text and get the special predefined key", function(assert) {
		var textKey = this.textPool.setText("", {
	...
	QUnit.test("Get (intermediate and persistent) Texts keys after creation of new texts", function(assert) {
		var keys, text, newKey, keyOnDB, sameNewKey;
		newKey = this.textPool.setText("Do not do it", {
			TextElementType : "YMESG",
			MaximumLength : 21,
			TranslationHint : "Just Do it"
		});
		...
		sameNewKey = this.textPool.setText("Do not do it", {
			TextElementType : "YMESG",
			MaximumLength : 21,
			TranslationHint : "Just Do it"
		});
	...
	/*
	QUnit.test("Error handling for non existing key", function() {
		var textPool = new sap.apf.modeler.core.TextPool(this.inject,  this.ApplicationId, "zy", this.texts);
		var newKey = textPool.setText("Do not do it", { TextElementType : "YMESG", MaximumLength : 21, TranslationHint : "Just Do it"});

		raises(function () {
		textPool.getPersistentKey('unknownKey');
		}, function( err ) {
			return err.toString() === "Error: TextPool: no textkey available for unknownKey";
		}, "Wrong key has been detected");
			
	});
	*/
	QUnit.test("GetTextsByTypeAndLength", function(assert) {
		this.textPool.setText("Do not do it", {
			TextElementType : "YMESG",
			MaximumLength : 21,
			TranslationHint : "Just Do it"
		});
		this.textPool.setText("Label1", {
			TextElementType : "XLAB",
			MaximumLength : 10,
			TranslationHint : "Nice"
		});
		this.textPool.setText("Label2", {
			TextElementType : "XLAB",
			MaximumLength : 10,
			TranslationHint : "Very Nice"
		});
		this.textPool.setText("Label3", {
			TextElementType : "XLAB",
			MaximumLength : 11,
			TranslationHint : "Nice"
		});
	...
	QUnit.test("Export Texts", function(assert) {
		...
		var fileString = this.textPool.exportTexts();
		...
		assert.deepEqual(fileString, expectedPropertyFile, "Text property file as expected");
	});
	...
	QUnit.test("Parse text property file with missing apf application id", function(assert) {
		...
		assert.equal(textFileInformation.Messages[0].code, 11010, "Expected error message");
	});
	QUnit.test("Parse text property file with missing entry", function(assert) {
		...
		assert.equal(textFileInformation.Messages[0].code, 11011, "Expected error message");
	});
	QUnit.test("Parse text property file with wrong guid format for TextElement", function(assert) {
		...
		assert.equal(textFileInformation.Messages[0].code, 11012, "Expected error message");
		assert.equal(textFileInformation.Messages[1].code, 11012, "Expected error message");
	});
	QUnit.module("M: Error wrong Application Id during export of textPool", {
		beforeEach : function() {
			...
			commonSetupTextPool(this, this.ApplicationId, this.expectedUuid);
		}
	});
	QUnit.test("Export Texts", function(assert) {
		var oDateFormat = sap.ui.core.format.DateFormat.getDateTimeInstance({
		...
		var fileString = this.textPool.exportTexts(assert);
		var expectedPropertyFile = "#FIORI: insert Fiori-Id\n" + "# __ldi.translation.uuid=" + this.expectedUuid + "\n" + "#ApfApplicationId=" + this.ApplicationId + "\n\n" + "#XLAB,15:Hint\n" + "343EC63F05550175E10000000A445B6D=uniqueLabelText\n"
				+ "# LastChangeDate=" + getDate(1412690202721) + "\n\n" + "#XTIT,30:Hint\n" + "143EC63F05550175E10000000A445B6D=TITLE1\n" + "# LastChangeDate=" + getDate(1412692222731) + "\n\n" + "#XTIT,30:Hint\n"
				+ "243EC63F05550175E10000000A445B6D=TITLE2\n" + "# LastChangeDate=" + getDate(1412692229733) + "\n\n";
		assert.deepEqual(fileString, expectedPropertyFile, "Text property file as expected");
		assert.equal(this.messageHandler.spyResults.putMessage.code, 11009, "Technical Error has been logged");
	});
	...
	QUnit.test("Export Texts", function(assert) {
		...
		var expectedPropertyFile = "#FIORI: insert Fiori-Id\n" + "# __ldi.translation.uuid=" + this.expectedUuid + "\n" + "#ApfApplicationId=" + this.ApplicationId + "\n\n" + "#XLAB,15:Hint\n" + "343EC63F05550175E10000000A445B6D=uniqueLabelText\n"
				...
				+ "243EC63F05550175E10000000A445B6D=TITLE2\n" + "# LastChangeDate=" + getDate(1412692229733) + "\n\n";
		assert.deepEqual(fileString, expectedPropertyFile, "Text property file as expected");
		assert.equal(this.messageHandler.spyResults.putMessage.code, 11008, "Technical Error has been logged");
	});
	...
	QUnit.module("M: TextPool - delete texts by guid", {
		beforeEach : function(assert) {
			...
			commonSetupTextPool(this, undefined, undefined, assertRemoveOperationIsOk);
		}
	});
