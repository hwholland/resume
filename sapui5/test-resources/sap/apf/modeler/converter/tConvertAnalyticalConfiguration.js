...
(function() {
					...
					"Setup " + WCAType,
					{
						...
						beforeEach : function(assert) {
							...
							that.generateBatchRequest = function(aCreateEntries) {
								var sXsrfToken = this.modelerCore.getXsrfToken(this.serviceRoot);
							...
							this.mapTextsFromRessourceBundleToTextPool = function(textKeyObjects) {
								...
								oBatchRequest = this.generateBatchRequest(textElements);
								var aTexts = [];
	...
	QUnit.test("Convert " + WCAType + " config", function(assert) {
		...
		that.modelerCore.getConfigurationHandler(that.applicationCreatedForTest, function(configurationHandler, messageObject) {
			...
			assert.ok(true, ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
			// save <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
			var textKey = textPool.setText(that.analyticalConfiguration.appDescription, sap.apf.modeler.ui.utils.TranslationFormatMap.APPLICATION_TITLE);
