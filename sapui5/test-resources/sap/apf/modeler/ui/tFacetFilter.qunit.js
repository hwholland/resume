...
(function() {
	...
	var spyOnAutoComplete, spyOnGetAllKnownProperties, spyOnGetOfTextPool, spyOnFFGetProperty, spyOnFFGetLabelKey, spyOnFFGetAlias, spyOnFFIsMultiSelection, spyOnFFVisibility, spyOnFFGetPreselectionFn, spyOnFFGetPreselectionDefaults, spyOnFFGetUseVHRAsFRR, spyOnSetValueList;
	var spyOnSetTextOfTextPool, spyOnConfigEditorSetIsUnsaved;
	...
	function _assertsForOnLoadOfNewFacetFilter(assert, label, property) {
		...
		var oDataForPropertyOrAlias = _dataForPropertyOrAlias();
		//assert
		assert.ok(oFacetFilterView, "then facet filter view exists");
		assert.strictEqual(spyOnAutoComplete.callCount, 3, "then autocomplete function is called thrice for facet filter view including sub views");
		assert.strictEqual(spyOnAutoComplete.calledWith(oFacetFilterView.byId("idLabel"), oDependenciesForService), true, "then autocomplete is enabled on facet filter label");
		...
		assert.strictEqual(oFacetFilterView.byId("idFFProperty").getValue(), property, "then property for facet filter is displayed");
		assert.deepEqual(oFacetFilterView.byId("idFFProperty").getModel().getData(), oDataForPropertyOrAlias, "then model data for property for facet filter is set");
		assert.strictEqual(oFacetFilterView.byId("idFFPropertyLabel").getRequired(), true, "then facet filter label is set required");
		assert.strictEqual(spyOnGetAllKnownProperties.calledTwice, true, "then all known properties are fetched");
	}
	...
	function _assertsForValueHelpRequest(assert, isVHRFieldInserted, isValueHelpRequest) {
		...
		assert.strictEqual(oFacetFilterView.byId("idFFAliasLabel").getVisible(), isValueHelpRequest, "then alias label visibility is" + isValueHelpRequest);
		assert.strictEqual(oFacetFilterView.byId("idFFAlias").getVisible(), isValueHelpRequest, "then alias input field visibility is" + isValueHelpRequest);
	}
	...
	function _assertsToCheckVisibility(assert) {
		...
		assert.strictEqual(oFacetFilterView.byId("idValueHelpTitle").getVisible(), true, "then value help title is set to visible");
	...
	function _assertsForInvisiblefilters(assert, VHRAndFormvisibility, isFRRVisible) {
		...
		assert.strictEqual(oFacetFilterView.byId("idValueHelpTitle").getVisible(), VHRAndFormvisibility, "then value help title invisibility is " + VHRAndFormvisibility);
	...
	function _commonTestForPreSelectionDefaultInvisibleToVisibleFilter(assert) {
		...
		assert.strictEqual(oFacetFilterView.byId("idFFAlias").getValue(), "", "then alias input field is set with empty value");
	...
	function _commonTestForPreselectionAutomaticValueInvisibleToVisibleFilter(assert) {
		//action
		oFacetFilterView.byId("idDoNotShowAtRuntimeCheckBox").setSelected(true);
		oFacetFilterView.getController().handleChangeForVisibilityAtRuntime();
		oFacetFilterView.byId("idDoNotShowAtRuntimeCheckBox").setSelected(false);
	...
	function _commonTestForInvisibleFilters(assert) {
		//arrange
		...
		_assertsForConfigListIfValues(assert, oFacetFilterView.byId("idVHNone"), false);
		assert.strictEqual(oFacetFilterView.byId("idUseVHRAsFRRCheckBox").getSelected(), false, "then use same as VHR checkbox is not selected");
		assert.strictEqual(spyFireEvent.calledWith("enableDisableFRRFieldsEvent"), true, "then enableDisableFRRFieldsEvent is fired when facet filter is invisible");
		assert.strictEqual(spyFireEvent.calledWith("doNotShowAtRuntimeEvent"), true, "then doNotShowAtRuntimeEvent is fired when facet filter is invisible");
	...
	function _commonTestForChangeOfPreSelectionfunctionToAutomaticValues(assert) {
		//arrange
		...
		_assertsForPreSelectionDefault(assert, false, "");
		_assertsForPreSelectionFunction(assert, false, "");
		assert.strictEqual(oFacetFilterView.byId("idValueHelpTitle").getVisible(), false, "then value help title is set to invisible");
	...
	function _commonSpiesWhileOnLoadOfFacetFilter(oModelerInstance) {
		...
		spyOnSetTextOfTextPool = sinon.spy(oTextPool, "setText");
		...
		spyOnGetAllKnownProperties = sinon.spy(oModelerInstance.configurationEditorForUnsavedConfig, "getAllKnownProperties");
		spyOnFFGetAlias = sinon.spy(oModelerInstance.facetFilterUnsaved2, "getAlias");
	...
	function _commonCleanUpsInAfterEach() {
		...
		oTextPool.setText.restore();
		oModelerInstance.configurationEditorForUnsavedConfig.getAllKnownProperties.restore();
	...
	function _dataForPropertyOrAlias() {
		return {
			"Objects" : [ {
				key : "property1",
				name : "property1"
	...
	QUnit.module("Test for a facet filter which is new", {
		...
		beforeEach : function(assert) {
			var done = assert.async(), oTextPoolHelper;
	...
	QUnit.test("When facet filter is initialized", function(assert) {
		...
		assert.strictEqual(oFacetFilterView.byId("idLabel").sId, focusedElement.parentElement.id, "Since the facet filter is new so facet filter title has focus on it ");
	...
	QUnit.test("When facet filter is set to invisible", function(assert) {
		_commonTestForInvisibleFilters(assert);
	});
	...
	QUnit.module("Test for a facet filter is visible, multiple selection, has automatic values and value help as Value help request ", {
		beforeEach : function(assert) {
			...
			sap.apf.testhelper.modelerUIHelper.getModelerInstance(function(modelerInstance) {
				...
				oModelerInstance.facetFilterUnsaved2.setAlias("Alias1");
	...
	QUnit.test("When facet filter is initialized", function(assert) {
		...
		_assertsForOnLoadOfFacetFilter(assert, true, oFacetFilterView.byId("idMultiSelectionMode"));
		assert.strictEqual(oFacetFilterView.byId("idDoNotShowAtRuntimeCheckBox").getSelected(), false, "then facet filter visibility is false");
		assert.deepEqual(document.body, focusedElement, "Since the facet filter is already saved with title so body has focus instead facet filter title");
		assert.strictEqual(oFacetFilterView.byId("idPreselectionModeRadioGroup").getSelectedButton(), oFacetFilterView.byId("idAutomaticSelection"), "then automatic selection is selected as preselection mode");
		_assertsForPreSelectionDefault(assert, false, "");
		_assertsForPreSelectionFunction(assert, false, "");
		_assertsForValueHelpRequest(assert, 0, true);
		assert.strictEqual(oFacetFilterView.byId("idFFAlias").getValue(), "Alias1", "then alias input field is set with value Alias1");
		_assertsForConfigListIfValues(assert, oFacetFilterView.byId("idValueHelpRequest"), false);
		assert.strictEqual(spyOnFFGetAlias.called, true, "then alias for facet filter is fetched");
		_assertsForUseVHRASFRR(assert, true, false);
		assert.strictEqual(spyOnFFGetUseVHRAsFRR.called, false, "then use VHR as FRR is not set");
	});
	QUnit.test("When preselection mode is automatic values and value help requests alias is changed", function(assert) {
		//arrange
		var spyOnFFSetAlias = sinon.spy(oModelerInstance.facetFilterUnsaved2, "setAlias");
		//action
		oFacetFilterView.byId("idFFAlias").setValue("alias2");
		oFacetFilterView.getController().handleChangeForAlias();
		//assert
		assert.strictEqual(spyOnFFSetAlias.calledWith("alias2"), true, "then setAlias is called on change of alias");
		assert.strictEqual(oModelerInstance.facetFilterUnsaved2.getAlias(), "alias2", "then the alias is set to the facet filter object");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.calledOnce, true, "then configuration editor is set to unsaved");
	});
	QUnit.test("When preselection mode is automatic values and value help requests alias is set to empty", function(assert) {
		//arrange
		var spyOnFFSetAlias = sinon.spy(oModelerInstance.facetFilterUnsaved2, "setAlias");
		//action
		oFacetFilterView.byId("idFFAlias").setValue("");
		oFacetFilterView.getController().handleChangeForAlias();
		//assert
		assert.strictEqual(spyOnFFSetAlias.calledWith(undefined), true, "then setAlias is called on change of alias to empty");
		assert.strictEqual(oModelerInstance.facetFilterUnsaved2.getAlias(), undefined, "then the alias is set as undefined to the facet filter object");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.calledOnce, true, "then configuration editor is set to unsaved");
	});
	...
	QUnit.test("When preselection mode is automatic values, value help is value help request and use same as VHR checkbox is selected", function(assert) {
		...
		assert.strictEqual(spyFireEvent.calledWith("useSameAsVHREvent"), true, "then useSameAsVHREvent is fired when use same as VHR is selected");
		assert.strictEqual(spyFireEvent.calledWith("enableDisableFRRFieldsEvent"), true, "then enableDisableFRRFieldsEvent is fired when use same as VHR is selected");
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.calledOnce, true, "then configuration editor is set to unsaved");
	});
	QUnit.test("When facet filter is set to invisible", function(assert) {
		//arrange
	...
	QUnit.test("When facet filter is initialized", function(assert) {
		...
		assert.strictEqual(oFacetFilterView.byId("idFFAliasLabel").getVisible(), false, "then label for alias is set not visible");
		assert.strictEqual(oFacetFilterView.byId("idFFAlias").getVisible(), false, "then alias input field is set not visible");
	...
	QUnit.test("When facet filter is set to invisible and changed to visible", function(assert) {
		_commonTestForPreselectionAutomaticValueInvisibleToVisibleFilter(assert);
	});
	...
	QUnit.test("When property of facet filter is changed", function(assert) {
		...
		//action
		oFacetFilterView.byId("idFFProperty").setValue("property1");
	...
	QUnit.test("When property of facet filter is changed to empty", function(assert) {
		...
		var spyOnFFSetProperty = sinon.spy(oModelerInstance.facetFilterUnsaved2, "setProperty");
		//action
		oFacetFilterView.byId("idFFProperty").setValue("");
		oFacetFilterView.getController().handleChangeForProperty();
		//assert
		assert.strictEqual(spyOnConfigEditorSetIsUnsaved.calledOnce, true, "then configuration editor is set to unsaved");
	...
	QUnit.test("When facet filter view data has to be reset then", function(assert) {
		...
		assert.strictEqual(spyFireEvent.calledWith("updateSubViewInstancesOnResetEvent"), true, "then sub views VHR and FRR are updated with new instances of editor and facet filter objects");
	});
	...
	QUnit.test("When preselection mode is changed from fixed values to automatic values", function(assert) {
		_commonTestForChangeOfPreSelectionfunctionToAutomaticValues(assert);
	});
	QUnit.test("When preselection mode is changed from fixed values to function and value help is none", function(assert) {
		//arrange
		var spyOnRemovePreselectionDefaults = sinon.spy(oModelerInstance.facetFilterUnsaved2, "removePreselectionDefaults");
