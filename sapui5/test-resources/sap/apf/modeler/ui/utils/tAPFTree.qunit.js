...
(function() {
	QUnit.module("APFTree - Unit Test", {
		beforeEach : function() {
			...
			this.oTreeInstance.bindAggregation("nodes", "/aConfigDetails", oTreeNodeTemplate);
	...
	QUnit.test("getAPFTreeNodeContext Test - with valid Configuration node ", function(assert) {
		var expectedContext = {
			...
			oNode : this.oTreeInstance.getNodes()[0]
		};
		var oNode = this.oTreeInstance.getNodes()[0];
	...
	QUnit.test("getAPFTreeNodeContext Test - with any valid subview node ", function(assert) {
		var expectedContext = {
			...
			nodeTitle : "Time",
			oNode : this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0]
		};
		var oNode = this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0];
		var nodeContext = this.oTreeInstance.getAPFTreeNodeContext(oNode);
	...
	QUnit.test("getParentNodeContext Test - by passing configuration node details  ", function(assert) {
		...
		var configNodeContext = this.oTreeInstance.getAPFTreeNodeContext(this.oTreeInstance.getNodes()[0]);
	...
	QUnit.test("getParentNodeContext Test - by passing facet filter node details  ", function(assert) {
		...
		var facetFilterNodeContext = this.oTreeInstance.getAPFTreeNodeContext(this.oTreeInstance.getNodes()[0].getNodes()[0].getNodes()[0]);
	...
	QUnit.test("getParentNodeContext Test - by passing category node details  ", function(assert) {
		...
		var categoryNodeContext = this.oTreeInstance.getAPFTreeNodeContext(this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0]);
	...
	QUnit.test("getParentNodeContext Test - by passing step node details  ", function(assert) {
		...
		var stepNodeContext = this.oTreeInstance.getAPFTreeNodeContext(this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0].getNodes()[0]);
	...
	QUnit.test("getParentNodeContext Test - by passing representation node details  ", function(assert) {
		...
		var repNodeContext = this.oTreeInstance.getAPFTreeNodeContext(this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0].getNodes()[0].getNodes()[0]);
	...
	QUnit.test("getParentNodeContext Test - by passing navigation target node details  ", function(assert) {
		...
		var navigationTargetNodeContext = this.oTreeInstance.getAPFTreeNodeContext(this.oTreeInstance.getNodes()[0].getNodes()[2].getNodes()[0]);
	...
	QUnit.test("isConfigurationSwitched Test - when current node and previous selected node are same", function(assert) {
		var currentSelectedNode = this.oTreeInstance.getNodes()[0];
		var previousSelectedNode = this.oTreeInstance.getNodes()[0];
	...
	QUnit.test("isConfigurationSwitched Test - when current node and previous selected node are different", function(assert) {
		var currentSelectedNode = this.oTreeInstance.getNodes()[1];
		var previousSelectedNode = this.oTreeInstance.getNodes()[0];
		var isConfigSwitched = this.oTreeInstance.isConfigurationSwitched(previousSelectedNode, currentSelectedNode);
	...
	QUnit.test("removeSelectionOnTree Test - when no selected node is passed to the function", function(assert) {
		this.oTreeInstance.setSelection(this.oTreeInstance.getNodes()[0]);
		this.oTreeInstance.removeSelectionOnTree();
		assert.deepEqual(this.oTreeInstance.getSelection(), null, "Selection has been removed from the tree");
	});
	QUnit.test("removeSelectionOnTree Test - when selected node is passed to the function", function(assert) {
		this.oTreeInstance.setSelection(this.oTreeInstance.getNodes()[0]);
		this.oTreeInstance.removeSelectionOnTree(this.oTreeInstance.getNodes()[0]);
		assert.deepEqual(this.oTreeInstance.getSelection(), null, "Selection has been removed from the tree");
		assert.deepEqual(this.oTreeInstance.getNodes()[0].getBindingContext().getObject().isSelected, false, "Selection property of selected node has been set to false");
	});
	QUnit.test("setSelectionOnTree Test", function(assert) {
		this.oTreeInstance.setSelection(this.oTreeInstance.getNodes()[1]);//config node 1 is already selected
		this.oTreeInstance.setSelectionOnTree(this.oTreeInstance.getNodes()[0].getBindingContext().sPath);//select config node 0
		assert.deepEqual(this.oTreeInstance.getSelection(), this.oTreeInstance.getNodes()[0], "Selection set properly on the tree");
	});
	QUnit.test("setSelectedNode Test", function(assert) {
		this.oTreeInstance.setSelectedNode(this.oTreeInstance.getNodes()[1]);
		assert.deepEqual(this.oTreeInstance.getSelection(), this.oTreeInstance.getNodes()[1], "Selection set correctly on to the tree");
	});
	...
	QUnit.test("addNodeInTree Test - Adding a new step node into the tree", function(assert) {
		this.oTreeInstance.setTranslationFunction(this.oCoreApi.getText);
		this.oTreeInstance.setSelection(this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0]);//to add a step selection on tree should be a category
		this.oTreeInstance.addNodeInTree("step", false);
		assert.equal(this.oModel.getData().aConfigDetails[0].configData[1].categories[0].steps.length, 2, "A new step added successfully into the tree");
	});
	QUnit.test("addNodeInTree Test - Adding an existing step into the tree", function(assert) {
		this.oTreeInstance.setTranslationFunction(this.oCoreApi.getText);
		this.oTreeInstance.setSelection(this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[1]);//to add a step selection on tree should be a category
		var params = {
	...
	QUnit.test("addNodeInTree Test - Adding a representation node into the tree", function(assert) {
		this.oTreeInstance.setTranslationFunction(this.oCoreApi.getText);
		this.oTreeInstance.setDefaultRepresentationType("Column Chart");
		this.oTreeInstance.setSelection(this.oTreeInstance.getNodes()[0].getNodes()[1].getNodes()[0].getNodes()[0]);//to add a representation selection on tree should be a step
