...
(function() {
	'use strict';
	QUnit.module("Modul sap.apf.utils.Hashtable", {
		beforeEach : function(assert) {
			this.oMsgHandler = new sap.apf.testhelper.doubles.MessageHandler().raiseOnCheck();
		},
	...
	QUnit.test("wrong arguments", function(assert) {
		assert.expect(3);
		var oHT = new sap.apf.utils.Hashtable(this.oMsgHandler);
		var i;
		for(i = 0; i < 7; i++) {
	...
	QUnit.test("each function", function(assert) {
		...
		oHT = new sap.apf.utils.Hashtable(this.oMsgHandler);
		for(i = 0; i < 10; i++) {
			oHT.setItem(i, i * 2);
		}
	...
	QUnit.test("item order - keys via ordering iterator", function(assert) {
		var hashtable = new sap.apf.utils.Hashtable(this.oMsgHandler);
	...
	QUnit.test("moveBefore", function(assert) {
		...
		getTheTestKeys().forEach(function(beforeIteratedKey) { // ["hugo", "otto", "mara", "anna"];
			getTheTestKeys().forEach(function(moved) {
				hashtable = new sap.apf.utils.Hashtable(that.oMsgHandler);
				fillHashtable(hashtable);
