...
(function() {
	...
	function commonSetup(testEnv) {
		...
		testEnv.messageHandler = new sap.apf.testhelper.doubles.MessageHandler().doubleCheckAndMessaging().spyPutMessage();
		...
		testEnv.filterIdHandler = new sap.apf.utils.FilterIdHandler(testEnv.inject);
	}
	...
	QUnit.test('Deserialize sets counter that does not conflict with existing internal IDs', function(assert) {
		...
		otherFilterIdHandler = new sap.apf.utils.FilterIdHandler(this.inject);
