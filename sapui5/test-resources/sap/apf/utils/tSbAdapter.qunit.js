jQuery.sap.require("sap.apf.utils.sbAdapter");
...
(function() {
	...
	function commonSetup (testEnv, assert){
		...
		sinon.stub(sap.ui.fl.LrepConnector, 'createConnector', function(){
	...
	function commonTeardown(){
		sap.ui.fl.LrepConnector.createConnector.restore();
		OData.request.restore();
		sap.apf.utils.sbAdapter._hanaConfigurations = undefined;
		sap.apf.utils.sbAdapter._lrepConfigurations = undefined;
	}
	...
	QUnit.test("No configurations in lrep and hana tables", function(assert){
		assert.expect(7);
		sap.apf.utils.sbAdapter.getConfigurations().done(function(result){
	...
	QUnit.test("Custom runtime intent given as parameters", function(assert){
		...
		sap.apf.utils.sbAdapter.getConfigurations("Semantic object", "Action").done(function(result){
	...
	QUnit.test("Suite on HANA intent given as parameter", function(assert){
		assert.expect(7);
		sap.apf.utils.sbAdapter.getConfigurations("FioriApplication", "executeAPFConfiguration").done(function(result){
	...
	QUnit.test("Incomplete intent given as parameter", function(assert){
		assert.expect(7);
		sap.apf.utils.sbAdapter.getConfigurations("SemanticObject").done(function(result){
			assert.deepEqual(result, this.standardExpected, "Array of configurations with generic runtime S4 intents returned");
		}.bind(this));
	});
	...
	QUnit.test("S4 intent given as parameter", function(assert){
		assert.expect(7);
		sap.apf.utils.sbAdapter.getConfigurations("FioriApplication", "executeAPFConfigurationS4HANA").done(function(result){
	...
	QUnit.test("Multiple calls, first with custom intent and then without ", function(assert){
		...
		sap.apf.utils.sbAdapter.getConfigurations().done(function(result){
			...
			sap.apf.utils.sbAdapter.getConfigurations("semanticObject", "action").done(function(result){
	...
	QUnit.test("No parameter", function (assert){
		assert.expect(3);
		sap.apf.utils.sbAdapter.getConfigurationNameById().done(function(configurationName){
	...
	QUnit.test("Not available S4 id given", function (assert){
		assert.expect(6);
		sap.apf.utils.sbAdapter.getConfigurationNameById("52135123.215123152").done(function(configurationName){
			assert.strictEqual(configurationName, "", "Empty String returned");
			assert.ok(!OData.request.called, "Odata never called");
	...
	QUnit.test("Available S4 id given", function (assert){
		assert.expect(6);
		sap.apf.utils.sbAdapter.getConfigurationNameById(this.appGuid + "." + this.configuration).done(function(configurationName){
	...
	QUnit.test("Not available SoH id given", function (assert){
		assert.expect(6);
		sap.apf.utils.sbAdapter.getConfigurationNameById("46845125").done(function(configurationName){
			assert.strictEqual(configurationName, "", "Empty String returned");
			assert.ok(OData.request.calledOnce, "Odata called once");
			assert.strictEqual(this.numberOfLrepCalls, 0, "Lrep never called");
		}.bind(this));
	...
	QUnit.test("Available SoH id given", function (assert){
		assert.expect(6);
		sap.apf.utils.sbAdapter.getConfigurationNameById(this.hanaConfig.AnalyticalConfiguration).done(function(configurationName){
	...
	QUnit.test("Function uses bufferd data", function (assert){
		assert.expect(11);
		sap.apf.utils.sbAdapter.getConfigurations().done(function(){
			assert.ok(OData.request.calledOnce, "Odata called once");
			assert.strictEqual(this.numberOfLrepCalls, 1, "Lrep called once");
			sap.apf.utils.sbAdapter.getConfigurationNameById(this.hanaConfig.AnalyticalConfiguration).done(function(configurationName){
				assert.strictEqual(configurationName, "SoHConfig", "Configuration name returned");
