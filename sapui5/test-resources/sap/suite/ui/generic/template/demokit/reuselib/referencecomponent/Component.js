...
sap.ui.define([
...
], function(UIComponent, JSONModel, ReuseComponentSupport) {
	"use strict";
	...
	return UIComponent.extend("reuselib.referencecomponent.Component", {
		...
		stStart: function(oModel, oBindingContext, oExtensionAPI) {
			 ...
			 */
			console.log(this.getId() + "->" + this.getMetadata().getName() + ":stStart  -> " + oBindingContext.getPath());
		...
		stRefresh: function(oModel, oBindingContext) {
			 ...
			 */
			console.log(this.getId() + "->" + this.getMetadata().getName() + ":stUpdate -> " + oBindingContext.getPath());
			this.updateHistory(oBindingContext);
		}
	});
}, /* bExport= */ true);