QUnit.module("Event handling", {
	beforeEach : function() {
		this.oVizFrameContent = new sap.suite.ui.commons.ChartContainerContent("chartContainerContent",{
			content : new sap.viz.ui5.controls.VizFrame("vizFrameEvents")
		});
		this.oChartContainer = new sap.suite.ui.commons.ChartContainer("chartContainerEvents", {
...
QUnit.test("_onShowLegendButtonPress calls legend button press method", function(assert) {
	...
	var oChartContainer = this.oChartContainer._onShowLegendButtonPress();
...
QUnit.test("_onChartSegmentButtonSelect switches the chart", function(assert) {
	...
	var oChartContainer = this.oChartContainer._onChartSegmentButtonSelect(oEvent);
...
QUnit.test("_onOverflowToolbarButtonPress fires button press with the correct props", function(assert) {
	...
	var oChartContainer = this.oChartContainer._onOverflowToolbarButtonPress(oEvent, oData);
...
QUnit.test("_onLegendButtonPress setShowLegend properly if getLegendVisible is not a method", function(assert) {
	...
	if (sap.ui.Device.browser.firefox === true) {
		assert.expect(0);
		return;
	}
	var bLegendVisible = true;
