...
(function () {
    ...
    /*global deepEqual, equal, module, ok, parseInt, stop, test, sinon, jQuery, sap, window */

    jQuery.sap.declare("sap.ushell.adapters.mock.UserDefaultParameterPersistenceAdapter");
    jQuery.sap.require("sap.ushell.services.UserDefaultParameterPersistence");
    ...
    /*
     * UserDefaultParameterPersistence Mock Adapter
     */
    sap.ushell.adapters.mock.UserDefaultParameterPersistenceAdapter = function () {
        this._oUserDefaultParameterPersistenceMap = new sap.ushell.utils.Map();
        this._oErrorMap = new sap.ushell.utils.Map();
    };

    sap.ushell.adapters.mock.UserDefaultParameterPersistenceAdapter.prototype.saveUserDefaultParameterPersistence = function (sKey, sSessionKey, sData, sAppname, sComponent) {
        var deferred = new jQuery.Deferred();
        if (!this._oErrorMap.containsKey(sKey)) {
            this._oUserDefaultParameterPersistenceMap.put(sKey, sData);
            deferred.resolve();
        } else {
            deferred.reject("Save of UserDefaultParameterPersistence failed");
        }
        return deferred.promise();
    };

    sap.ushell.adapters.mock.UserDefaultParameterPersistenceAdapter.prototype.loadUserDefaultParameterPersistence = function (sKey) {
        var deferred = new jQuery.Deferred();
        if (!this._oErrorMap.containsKey(sKey) && this._oUserDefaultParameterPersistenceMap.containsKey(sKey)) {
            deferred.resolve(sKey, this._oUserDefaultParameterPersistenceMap.get(sKey));
        } else {
    ...
    test("loadParameterValue", function () {
        ...
        oService = new sap.ushell.services.UserDefaultParameterPersistence(oAdapter);
    ...
    test("getStoredParameterNames", function () {
         var oService,
             oAdapter = {
                 "getStoredParameterNames" : function() {}
             };
         oService = new sap.ushell.services.UserDefaultParameterPersistence(oAdapter);
    ...
    test("save and load value undefined, delete called!", function () {
        ...
        sinon.stub(oAdapter, "loadParameterValue", function (sParameterName) { return new jQuery.Deferred().resolve({ value : "fakeLoad"}).promise(); });
