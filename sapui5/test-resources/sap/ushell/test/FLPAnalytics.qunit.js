// @copyright@
/**
 * @fileOverview QUnit tests for sap.ushell.components.flp.FLPAnalytics
 ...
 *  The idea is to publish the relevant event (e.g. appOpened, bookmarkTileAdded, etc..) and to check the corresponding call
 *   to UsageAnalytics service function logCustomEvent.
 *  Also: in each test case, the hash is being set using window.location.hash,
 *   and the test also verifies that window.swa.custom1.ref got the correct hash value
 */
...
(function () {
    ...
    jQuery, sap, hasher, sinon*/

    jQuery.sap.require("sap.ushell.services.Container");
    jQuery.sap.require("sap.ushell.ui.launchpad.ViewPortContainer");
    jQuery.sap.require("sap.ui.core.theming.Parameters");

    var oNavContainer = new sap.ushell.ui.launchpad.ViewPortContainer({
    ...
    module("sap.ushell.components.flp.FLPAnalytics", {
        setup: function () {
            ...
            oGetServiceStub.returns({
                logCustomEvent : oUsageAnalyticsLogStub
            });
    ...
    test("Open application logging", function () {
        ...
        sap.ui.getCore().getEventBus().publish("launchpad", "appOpened", {});
        ...
        ok(oUsageAnalyticsLogStub.args[0][2][0] === "appTitle0", "logCustomEvent called with application title 'appTitle1'");
    });
    ...
    test("Most used source logging", function () {
        ...
        for (index = 10; index < 15; index++) {
            oData = {
                targetHash: "#businessObject" + index + "-action" + index
            };
            sap.ui.getCore().getEventBus().publish("launchpad", "dashboardTileLinkClick", oData);
        }

        ok(window.swa.custom1.ref === "businessObject14-action14", "swa.custom1.ref contains the last hash - 'businessObject14-action14'");
