...
sap.ui.define(["sap/base/storage"], function(oStorageModule) {
	"use strict";
	var storage = oStorageModule(window);
	var storageType = oStorageModule.Type;
	...
	QUnit.test("Global Storage", function(assert) {
		setup.testStorage(storageType.global, "myprefix", sap.ui.Device.os.ios ? setup.SUPPORTED_CHECK_INACTIVE : setup.SUPPORTED_CHECK_EXPECT_NO_SUPPORT, assert);
	});
	...
	QUnit.test("Cache", function(assert) {
		...
		setup.testCache(storageType.global, "myprefix", true, assert);
